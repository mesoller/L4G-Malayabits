...
try:
    ...
    console.print(
        "[bold red]Missing Module Found. Please reinstall required dependancies."
    )
...
class Prep:
    ...
    async def check_images_concurrently(self, imagelist):
        async def check_and_collect(image_dict):
            ...
            if img_url and await self.check_image_link(img_url):
                ...
                console.print(
                    f"[yellow]Image link failed verification and will be skipped: {img_url}[/yellow]"
                )
                return None
                    ...
                    if response.status == 200:
                        ...
                        if "image" in content_type:
                            ...
                            try:
                                ...
                                console.print(
                                    f"[green]Image verified successfully: {url}[/green]"
                                )
                                return True
                            except (IOError, SyntaxError) as e:  # noqa #F841
                                console.print(
                                    f"[red]Image verification failed (corrupt image): {url}[/red]"
                                )
                                return False
                        else:
                            console.print(
                                f"[red]Content type is not an image: {url}[/red]"
                            )
                            return False
                    else:
                        console.print(
                            f"[red]Failed to retrieve image: {url} (status code: {response.status})[/red]"
                        )
                        return False
            except Exception as e:
                console.print(
                    f"[red]Exception occurred while checking image: {url} - {str(e)}[/red]"
                )
                return False
    ...
    async def update_meta_with_unit3d_data(self, meta, tracker_data, tracker_name):
        ...
        tmdb, imdb, tvdb, mal, desc, category, infohash, imagelist, filename, *rest = (
            tracker_data
        )
        ...
        if category.upper() in ["MOVIE", "TV SHOW", "FANRES"]:
            meta["category"] = (
                "TV" if category.upper() == "TV SHOW" else category.upper()
            )

        if not meta.get(
            "image_list"
        ):  # Only handle images if image_list is not already populated
            if imagelist:  # Ensure imagelist is not empty before setting
                ...
                if valid_images:
                    ...
                    if meta.get(
                        "image_list"
                    ):  # Double-check if image_list is set before handling it
                        if (
                            not (
                                meta.get("blu")
                                or meta.get("aither")
    ...
    async def update_metadata_from_tracker(
        self, tracker_name, tracker_instance, meta, search_term, search_file_folder
    ):
        ...
        if tracker_name in ["BLU", "AITHER", "LST", "OE", "TIK"]:
            if meta.get(tracker_key) is not None:
                console.print(
                    f"[cyan]{tracker_name} ID found in meta, reusing existing ID: {meta[tracker_key]}[/cyan]"
                )
            ...
            if any(
                ...
                console.print(
                    f"[green]Valid data found on {tracker_name}, setting meta values[/green]"
                )
                await self.update_meta_with_unit3d_data(
                    meta, tracker_data, tracker_name
                )
                found_match = True
            else:
            ...
            if meta.get("ptp") is None:
                ...
                imdb_id, ptp_torrent_id, ptp_torrent_hash = (
                    await tracker_instance.get_ptp_id_imdb(
                ...
                if ptp_torrent_id:
                    ...
                    console.print(
                        f"[green]{tracker_name} IMDb ID found: tt{meta['imdb']}[/green]"
                    )
                    if not meta["unattended"]:
                        if await self.prompt_user_for_confirmation(
                            "Do you want to use this ID data from PTP?"
                        ):
                            meta["skip_gen_desc"] = True
                            found_match = True

                            # Retrieve PTP description and image list
                            ptp_desc, ptp_imagelist = (
                                await tracker_instance.get_ptp_description(
                                    ptp_torrent_id, meta, meta.get("is_disc", False)
                                )
                            )
                            ...
                            if not meta.get(
                                ...
                                valid_images = await self.check_images_concurrently(
                                    ptp_imagelist
                                )
                                if valid_images:
                                    meta["image_list"] = valid_images
                            ...
                            meta["skip_gen_desc"] = True
                            console.print(
                                "[green]PTP images added to metadata.[/green]"
                            )
                ...
                console.print(
                    f"[cyan]PTP ID found in meta: {ptp_torrent_id}, using it to get IMDb ID[/cyan]"
                )
                imdb_id, _, meta["ext_torrenthash"] = (
                    await tracker_instance.get_imdb_from_torrent_id(ptp_torrent_id)
                )
                if imdb_id:
                    ...
                    console.print(
                        f"[yellow]Could not find IMDb ID using PTP ID: {ptp_torrent_id}[/yellow]"
                    )
                    found_match = False

                # Retrieve PTP description and image list
                ptp_desc, ptp_imagelist = await tracker_instance.get_ptp_description(
                    meta["ptp"], meta, meta.get("is_disc", False)
                )
                meta["description"] = ptp_desc
            ...
            if meta.get("hdb") is not None:
                ...
                imdb, tvdb_id, hdb_name, meta["ext_torrenthash"] = (
                    await tracker_instance.get_info_from_torrent_id(meta[tracker_key])
                )
                ...
                console.print(
                    f"[green]{tracker_name} data found: IMDb ID: {imdb}, TVDb ID: {meta['tvdb_id']}, HDB Name: {meta['hdb_name']}[/green]"
                )
            else:
                console.print(
                    "[yellow]No ID found in meta for HDB, searching by file name[/yellow]"
                )

                # Use search_filename function if ID is not found in meta
                imdb, tvdb_id, hdb_name, meta["ext_torrenthash"], tracker_id = (
                    await tracker_instance.search_filename(
                        search_term, search_file_folder, meta
                    )
                )
    ...
    async def handle_image_list(self, meta, tracker_name):
        if meta.get("image_list"):
            ...
            if all(
                any(host in image["raw_url"] for host in approved_image_hosts)
                ...
                trackers_list = [
                    tracker.strip() for tracker in default_trackers.split(",")
                ]
                if "MTV" in trackers_list or "MTV" in meta.get("trackers", ""):
                    console.print(
                        "[red]Warning: Some images are not hosted on an MTV approved image host. MTV will fail if you keep these images."
                    )

            if meta["unattended"]:
                keep_images = True
            else:
                keep_images = await self.prompt_user_for_confirmation(
    ...
    async def gather_prep(self, meta, mode):
        ...
        if meta["is_disc"] == "BDMV":
            video, meta["scene"], meta["imdb"] = self.is_scene(
                meta["path"], meta.get("imdb", None)
            )
            ...
            if meta.get("resolution", None) is None:
                meta["resolution"] = self.mi_resolution(
                    bdinfo["video"][0]["res"],
                    guessit(video),
                    width="OTHER",
                    scan="p",
            ...
            filename = guessit(guess_name, {"excludes": ["country", "language"]})[
                "title"
            ]
            untouched_filename = os.path.basename(
                os.path.dirname(meta["discs"][0]["path"])
            )
            try:
            ...
            if not meta.get("edit", False):
                mi = self.exportInfo(
                    f"{meta['discs'][0]['path']}/VTS_{meta['discs'][0]['main_set'][0][:2]}_1.VOB",
                    False,
                    meta["uuid"],
                    meta["base_dir"],
                    export_text=False,
            ...
            meta["dvd_size"] = await self.get_dvd_size(
                meta["discs"], meta.get("manual_dvds")
            )
            meta["resolution"] = self.get_resolution(
                guessit(video), meta["uuid"], base_dir
            )
            meta["sd"] = self.is_sd(meta["resolution"])
            ...
            videopath, meta["filelist"] = self.get_video(
                videoloc, meta.get("mode", "discord")
            )
            search_term = (
                os.path.basename(meta["filelist"][0]) if meta["filelist"] else None
            )
            search_file_folder = "file"
            video, meta["scene"], meta["imdb"] = self.is_scene(
                videopath, meta.get("imdb", None)
            )
            ...
            if not meta.get("edit", False):
                mi = self.exportInfo(
                    videopath, meta["isdir"], meta["uuid"], base_dir, export_text=True
                )
                meta["mediainfo"] = mi
            else:
                mi = meta["mediainfo"]
        ...
        if not meta.get("image_list"):
            ...
            if search_term:
                ...
                if specific_tracker:
                    console.print(
                        f"[blue]Processing only the {specific_tracker} tracker based on meta.[/blue]"
                    )

                    if (
                        specific_tracker == "PTP"
                        and str(
                            self.config["TRACKERS"].get("PTP", {}).get("useAPI")
                        ).lower()
                        == "true"
                    ):
                        ptp = PTP(config=self.config)
                        meta, match = await self.update_metadata_from_tracker(
                            "PTP", ptp, meta, search_term, search_file_folder
                        )
                        if match:
                            found_match = True
                        ...
                        specific_tracker == "BLU"
                        and str(
                            self.config["TRACKERS"].get("BLU", {}).get("useAPI")
                        ).lower()
                        == "true"
                    ):
                        blu = BLU(config=self.config)
                        meta, match = await self.update_metadata_from_tracker(
                            "BLU", blu, meta, search_term, search_file_folder
                        )
                        if match:
                            found_match = True
                        ...
                        specific_tracker == "AITHER"
                        and str(
                            self.config["TRACKERS"].get("AITHER", {}).get("useAPI")
                        ).lower()
                        == "true"
                    ):
                        aither = AITHER(config=self.config)
                        meta, match = await self.update_metadata_from_tracker(
                            "AITHER", aither, meta, search_term, search_file_folder
                        )
                        if match:
                            found_match = True
                        ...
                        specific_tracker == "LST"
                        and str(
                            self.config["TRACKERS"].get("LST", {}).get("useAPI")
                        ).lower()
                        == "true"
                    ):
                        lst = LST(config=self.config)
                        meta, match = await self.update_metadata_from_tracker(
                            "LST", lst, meta, search_term, search_file_folder
                        )
                        if match:
                            found_match = True
                        ...
                        specific_tracker == "OE"
                        and str(
                            self.config["TRACKERS"].get("OE", {}).get("useAPI")
                        ).lower()
                        == "true"
                    ):
                        oe = OE(config=self.config)
                        meta, match = await self.update_metadata_from_tracker(
                            "OE", oe, meta, search_term, search_file_folder
                        )
                        if match:
                            found_match = True
                        ...
                        specific_tracker == "TIK"
                        and str(
                            self.config["TRACKERS"].get("TIK", {}).get("useAPI")
                        ).lower()
                        == "true"
                    ):
                        tik = TIK(config=self.config)
                        meta, match = await self.update_metadata_from_tracker(
                            "TIK", tik, meta, search_term, search_file_folder
                        )
                        if match:
                            found_match = True
                        ...
                        specific_tracker == "HDB"
                        and str(
                            self.config["TRACKERS"].get("HDB", {}).get("useAPI")
                        ).lower()
                        == "true"
                    ):
                        hdb = HDB(config=self.config)
                        meta, match = await self.update_metadata_from_tracker(
                            "HDB", hdb, meta, search_term, search_file_folder
                        )
                        if match:
                            found_match = True
                else:
                    ...
                    default_trackers = (
                        self.config["TRACKERS"].get("default_trackers", "").split(", ")
                    )
        ...
        if meta["is_disc"] == "BDMV":
            if meta.get("edit", False) is False:
                if meta.get("vapoursynth", False) is True:
                    ...
                    use_vs = False
                try:
                    ds = multiprocessing.Process(
                        target=self.disc_screenshots,
                        args=(
                            filename,
                            bdinfo,
                            meta["uuid"],
            ...
            if meta.get("edit", False) is False:
                try:
                    ds = multiprocessing.Process(
                        target=self.dvd_screenshots, args=(meta, 0, None)
                    )
                    ds.start()
                    while ds.is_alive() is True:
                        await asyncio.sleep(1)
            ...
            if meta.get("edit", False) is False:
                try:
                    s = multiprocessing.Process(
                        ...
                        args=(videopath, filename, meta["uuid"], base_dir, meta),
                    )
                    s.start()
        ...
        if meta.get("tmdb", None) is None and meta.get("imdb", None) is None:
            meta["category"], meta["tmdb"], meta["imdb"] = (
                self.get_tmdb_imdb_from_mediainfo(
                    mi, meta["category"], meta["is_disc"], meta["tmdb"], meta["imdb"]
                )
            )
        if meta.get("tmdb", None) is None and meta.get("imdb", None) is None:
            meta = await self.get_tmdb_id(
                filename,
                meta["search_year"],
                meta,
                meta["category"],
            ...
            meta.get("imdb", None) is not None and meta.get("tmdb_manual", None) is None
        ):
        ...
        # Search tvmaze
        meta["tvmaze_id"], meta["imdb_id"], meta["tvdb_id"] = await self.search_tvmaze(
            filename,
            meta["search_year"],
            meta.get("imdb_id", "0"),
            meta.get("tvdb_id", 0),
        ...
        meta["video"] = video
        meta["audio"], meta["channels"], meta["has_commentary"] = self.get_audio_v2(
            mi, meta, bdinfo
        )
        ...
        meta["source"], meta["type"] = self.get_source(
            meta["type"], video, meta["path"], meta["is_disc"], meta
        )
        if meta.get("service", None) in (None, ""):
            meta["service"], meta["service_longname"] = self.get_service(
                video, meta.get("tag", ""), meta["audio"], meta["filename"]
            )
        elif meta.get("service"):
            services = self.get_service(get_services_only=True)
            meta["service_longname"] = max(
                (k for k, v in services.items() if v == meta["service"]),
                key=len,
                default=meta["service"],
            )
        meta["uhd"] = self.get_uhd(
            meta["type"], guessit(meta["path"]), meta["resolution"], meta["path"]
        )
        ...
        if meta.get("is_disc", None) == "BDMV":  # Blu-ray Specific
            ...
            (
                ...
                meta["bit_depth"],
            ) = self.get_video_encode(mi, meta["type"], bdinfo)

        meta["edition"], meta["repack"] = self.get_edition(
            meta["path"], bdinfo, meta["filelist"], meta.get("manual_edition")
        )
    ...
    async def get_disc(self, meta):
        ...
        if is_disc == "BDMV":
            if meta.get("edit", False) is False:
                discs, bdinfo = await parse.get_bdinfo(
                    discs, meta["uuid"], meta["base_dir"], meta.get("discs", [])
                )
            else:
                discs, bdinfo = await parse.get_bdinfo(
    ...
    def get_video(self, videoloc, mode):
        ...
        if os.path.isdir(videoloc):
            globlist = (
                glob.glob1(videoloc, "*.mkv")
                + glob.glob1(videoloc, "*.mp4")
                + glob.glob1(videoloc, "*.ts")
            )
    ...
    def exportInfo(self, video, isdir, folder_id, base_dir, export_text):
        def filter_mediainfo(data):
            filtered = {
                ...
                "media": {"@ref": data["media"]["@ref"], "track": []},
            }
        ...
        if (
            not os.path.exists(f"{base_dir}/tmp/{folder_id}/MEDIAINFO.txt")
            and export_text
        ):
            ...
            with open(
                ...
                export_cleanpath.write(
                    media_info.replace(video, os.path.basename(video))
                )
            console.print("[bold green]MediaInfo Exported.")

        if not os.path.exists(f"{base_dir}/tmp/{folder_id}/MediaInfo.json.txt"):
            media_info_json = MediaInfo.parse(
                video, output="JSON", mediainfo_options={"inform_version": "1"}
            )
            media_info_dict = json.loads(media_info_json)
            filtered_info = filter_mediainfo(media_info_dict)
            with open(
                f"{base_dir}/tmp/{folder_id}/MediaInfo.json", "w", encoding="utf-8"
            ) as export:
                json.dump(filtered_info, export, indent=4)

        with open(
            f"{base_dir}/tmp/{folder_id}/MediaInfo.json", "r", encoding="utf-8"
        ) as f:
            mi = json.load(f)
    ...
    def get_resolution(self, guess, folder_id, base_dir):
        with open(
            ...
            resolution = self.mi_resolution(
                res, guess, width, scan, height, actual_height
            )
        return resolution
    ...
    def mi_resolution(self, res, guess, width, scan, height, actual_height):
        ...
        if resolution is None:
            ...
            resolution = self.mi_resolution(
                resolution, guess, width, scan, height, actual_height
            )
        ...
        try:
            ...
            if int(response.get("resultsCount", 0)) != 0:
                ...
                if r["releases"] != [] and imdb is None:
                    imdb = (
                        r["releases"][0].get("imdb", imdb)
                        if r["releases"][0].get("imdb") is not None
                        else imdb
                    )
                console.print(
                    f"[green]SRRDB: Matched to {response['results'][0]['release']}"
                )
        except Exception:
            video = video
            scene = False
    ...
    def disc_screenshots(
        ...
        for each in bdinfo["files"]:
            int_length = sum(
                int(float(x)) * 60**i
                for i, x in enumerate(reversed(each["length"].split(":")))
            )
        ...
        if i >= num_screens:
            ...
            if use_vs is True:
                ...
                vs_screengn(
                    source=file,
                    encode=None,
                    filter_b_frames=False,
                    num=num_screens,
                    dir=f"{base_dir}/tmp/{folder_id}/",
                )
            else:
                if bool(ffdebug) is True:
                    ...
                    with Progress(
                        ...
                        screen_task = progress.add_task(
                            "[green]Saving Screens...", total=num_screens + 1
                        )
                        ss_times = []
                        for i in range(num_screens + 1):
                            ...
                            try:
                                ss_times = self.valid_ss_time(
                                    ss_times, num_screens + 1, length
                                )
                                (
                                    ffmpeg.input(
                                        file, ss=ss_times[-1], skip_frame=keyframe
                                    )
                            ...
                            if (
                                os.path.getsize(Path(image)) <= 31000000
                                and self.img_host == "imgbb"
                            ):
                                i += 1
                            elif os.path.getsize(
                                Path(image)
                            ) <= 10000000 and self.img_host in ["imgbox", "pixhost"]:
                                i += 1
                            elif os.path.getsize(Path(image)) <= 75000:
                                console.print(
                                    "[bold yellow]Image is incredibly small, retaking"
                                )
                                time.sleep(1)
                                ...
                                console.print(
                                    "[red]Image too large for your image host, retaking"
                                )
                                time.sleep(1)
                ...
                smallest = None
                smallestsize = 99**99
                for screens in glob.glob1(
                    f"{base_dir}/tmp/{folder_id}/", f"{filename}-*"
                ):
    ...
    def dvd_screenshots(self, meta, disc_num, num_screens=None):
        if num_screens is None:
            num_screens = self.screens
        if num_screens == 0 or (
            len(meta.get("image_list", [])) >= num_screens and disc_num == 0
        ):
            return
        ...
        if (
            len(
                glob.glob(
                    f"{meta['base_dir']}/tmp/{meta['uuid']}/{meta['discs'][disc_num]['name']}-*.png"
                )
            )
            >= num_screens
        ):
                ...
                for i in range(num_screens + 1):
                    ...
                    if not os.path.exists(image) or retake is not False:
                        ...
                        def _is_vob_good(n, loops, num_screens):
                            ...
                            try:
                                voblength = float(
                                    vob_mi["media"]["track"][1]["Duration"]
                                )
                                return voblength, n
                            except Exception:
                                try:
                                    voblength = float(
                                        vob_mi["media"]["track"][2]["Duration"]
                                    )
                                    return voblength, n
                                except Exception:
                                    n += 1
                        ...
                        try:
                            ...
                            ss_times = self.valid_ss_time(
                                ss_times, num_screens + 1, voblength
                            )
                            ff = ffmpeg.input(
                                f"{meta['discs'][disc_num]['path']}/VTS_{main_set[n]}",
                                ss=ss_times[-1],
                            ...
                            if w_sar != 1 or h_sar != 1:
                                ff = ff.filter(
                                    "scale",
                                    int(round(width * w_sar)),
                                    int(round(height * h_sar)),
                                )
                            (
                                ff.output(image, vframes=1, pix_fmt="rgb24")
                                .overwrite_output()
                                .global_args("-loglevel", loglevel)
                                .run(quiet=debug)
                            )
                        ...
                        try:
                            if (
                                ...
                                console.print(
                                    "[yellow]Image is incredibly small (and is most likely to be a single color), retaking"
                                )
                                retake = True
                                time.sleep(1)
                            elif self.img_host == "ptpimg":
            ...
            for screens in glob.glob1(
                ...
                f"{meta['discs'][disc_num]['name']}-*",
            ):
                screen_path = os.path.join(
                    f"{meta['base_dir']}/tmp/{meta['uuid']}/", screens
                )
                screensize = os.path.getsize(screen_path)
                if screensize < smallestsize:
                    smallestsize = screensize
    ...
    def screenshots(
        ...
        existing_images = [
            img
            for img in meta["image_list"]
            if isinstance(img, dict) and img.get("img_url", "").startswith("http")
        ]
        ...
        if len(existing_images) >= 3 and not force_screenshots:
            console.print(
                "[yellow]There are already at least 3 images in the image list. Skipping additional screenshots."
            )
            return
                    ...
                    with Progress(
                        ...
                        for i in range(num_screens + 1):
                            image_path = os.path.abspath(
                                f"{base_dir}/tmp/{folder_id}/{filename}-{i}.png"
                            )
                            if not os.path.exists(image_path) or retake is not False:
                                ...
                                try:
                                    ...
                                    (
                                        ff.output(
                                            image_path, vframes=1, pix_fmt="rgb24"
                                        )
                                        .overwrite_output()
                                        .global_args("-loglevel", loglevel)
                                        .run(quiet=debug)
                                ...
                                if os.path.getsize(Path(image_path)) <= 75000:
                                    console.print(
                                        "[yellow]Image is incredibly small, retaking"
                                    )
                                    retake = True
                                    time.sleep(1)
                                if (
                                    os.path.getsize(Path(image_path)) <= 31000000
                                    and self.img_host == "imgbb"
                                    and retake is False
                                ):
                                    i += 1
                                elif (
                                    os.path.getsize(Path(image_path)) <= 10000000
                                    and self.img_host in ["imgbox", "pixhost"]
                                    and retake is False
                                ):
                                    i += 1
                                elif (
                                    self.img_host in ["ptpimg", "lensdump", "ptscreens"]
                                    and retake is False
                                ):
                                    i += 1
                                elif self.img_host == "freeimage.host":
                                    console.print(
                                        "[bold red]Support for freeimage.host has been removed. Please remove from your config"
                                    )
                                    exit()
                                elif retake is True:
                                    pass
                        ...
                        if len(meta["image_list"]) > self.screens:
                            local_images = [
                                img
                                for img in meta["image_list"]
                                if not img["img_url"].startswith("http")
                            ]

                            if local_images:
                                smallest = min(
                                    local_images,
                                    key=lambda x: os.path.getsize(x["img_url"]),
                                )
                                os.remove(smallest["img_url"])
                                meta["image_list"].remove(smallest)
                            else:
                                console.print(
                                    "[yellow]No local images found to remove."
                                )
    ...
    async def get_tmdb_from_imdb(self, meta, filename):
        ...
        if len(info["movie_results"]) >= 1:
            ...
            console.print(
                f"[yellow]TMDb was unable to find anything with that IMDb, searching TMDb for {title}"
            )
            meta = await self.get_tmdb_id(
                title,
                year,
                ...
                meta["category"],
                imdb_info.get(
                    "original title", imdb_info.get("localized title", meta["uuid"])
                ),
            )
    ...
    async def get_tmdb_id(
        self, filename, search_year, meta, category, untouched_filename="", attempted=0
    ):
        search = tmdb.Search()
        try:
            if category == "MOVIE":
    ...
    async def tmdb_other_meta(self, meta):
        ...
        if meta["tmdb"] == "0":
            try:
                title = guessit(meta["path"], {"excludes": ["country", "language"]})[
                    "title"
                ].lower()
        ...
        if meta["category"] == "MOVIE":
            ...
            if response["release_date"]:
                meta["year"] = datetime.strptime(
                    response["release_date"], "%Y-%m-%d"
                ).year
            else:
                console.print(
                    "[yellow]TMDB does not have a release date, using year from filename instead (if it exists)"
                )
            ...
            try:
                ...
                for each in videos.get("results", []):
                    if (
                        each.get("site", "") == "YouTube"
                        and each.get("type", "") == "Trailer"
                    ):
                        meta["youtube"] = (
                            f"https://www.youtube.com/watch?v={each.get('key')}"
                        )
                        break
            except Exception:
            ...
            if meta.get("anime", False) is False:
                meta["mal_id"], meta["aka"], meta["anime"] = self.get_anime(
                    response, meta
                )
        ...
        difference = SequenceMatcher(
            None, meta["title"].lower(), meta["aka"][5:].lower()
        ).ratio()
        if (
            difference >= 0.9
            or meta["aka"][5:].strip() == ""
            or meta["aka"][5:].strip().lower() in meta["title"].lower()
        ):
            meta["aka"] = ""
    ...
    def get_anime(self, response, meta):
        ...
        if response["original_language"] == "ja" and animation is True:
            romaji, mal_id, eng_title, season_year, episodes = self.get_romaji(
                tmdb_name, meta.get("mal", None)
            )
    ...
    def get_romaji(self, tmdb_name, mal):
        ...
        if media not in (None, []):
            ...
            eng_title = result["title"].get(
                "english", result["title"].get("romaji", "")
            )
    ...
    def get_audio_v2(self, mi, meta, bdinfo):
        ...
        if bdinfo is not None:  # Disks
            ...
            commercial = format
            additional = bdinfo.get("audio", [{}])[0].get(
            ...
            for i, t in enumerate(tracks):
                if t.get("@type") != "Audio":
                    continue
                if (
                    t.get("Language", "") == meta.get("original_language", "")
                    and "commentary" not in t.get("Title", "").lower()
                ):
                    track_num = i
                    break
            ...
            if format_settings in ["Explicit"]:
                format_settings = ""
            # Channels
            channels = mi["media"]["track"][track_num].get(
                "Channels_Original", mi["media"]["track"][track_num]["Channels"]
            )
            ...
            try:
                ...
                try:
                    channel_layout = mi["media"]["track"][track_num][
            ...
            if meta.get("original_language", "") != "en":
                ...
                try:
                    for t in mi.get("media", {}).get("track", []):
                        ...
                        if (
                            audio_language == meta["original_language"]
                            and "commentary" not in t.get("Title", "").lower()
                        ):
                            orig = True
                        ...
                        if (
                            audio_language in variants
                            and meta["original_language"] in variants
                        ):
                            orig = True
                        ...
                        if (
                            ...
                            if (
                                meta["original_language"] not in variants
                                and audio_language not in variants
                            ):
                                audio_language = (
                                    "und" if audio_language == "" else audio_language
                                )
                                console.print(
                                    f"[bold red]This release has a(n) {audio_language} audio track, and may be considered bloated"
                                )
                                time.sleep(5)
                    ...
                    if eng and orig:
                        ...
                        eng
                        and not orig
                        and meta["original_language"] not in ["zxx", "xx", None]
                        and not meta.get("no_dub", False)
                    ):
                        dual = "Dubbed"
                except Exception:
                    console.print(traceback.format_exc())
        ...
        if commercial:
            for key, value in commercial_names.items():
                ...
                if (
                    "Atmos" in commercial
                    or format_extra.get(additional, "") == " Atmos"
                ):
                    extra = " Atmos"
        ...
        audio = (
            f"{dual} {codec or ''} {format_settings or ''} {chan or ''}{extra or ''}"
        )
        ...
        if bdinfo is not None:  # Disks
            ...
            try:
                ...
                if hdr_mi in ("BT.2020", "REC.2020"):
                    ...
                    hdr_format_string = video_track.get(
                        "HDR_Format_Compatibility",
                        video_track.get(
                            "HDR_Format_String", video_track.get("HDR_Format", "")
                        ),
                    )
                    ...
                    if hdr_format_string == "" and "PQ" in (
                        ...
                        video_track.get("transfer_characteristics_Original", None),
                    ):
                        hdr = "PQ10"
                    transfer_characteristics = video_track.get(
            ...
            try:
                if "Dolby Vision" in video_track.get(
                    "HDR_Format", ""
                ) or "Dolby Vision" in video_track.get("HDR_Format_String", ""):
                    dv = "DV"
            except Exception:
                pass
    ...
    def get_region(self, bdinfo, region=None):
        label = bdinfo.get(
            "label", bdinfo.get("title", bdinfo.get("path", ""))
        ).replace(".", " ")
    ...
    def get_edition(self, video, bdinfo, filelist, manual_edition):
        ...
        video = (
            video.upper().replace(".", " ").replace(tag.upper(), "").replace("-", "")
        )
        ...
        edition = re.sub(
            r"(\bREPACK\d?\b|\bRERIP\b|\bPROPER\b)", "", edition, flags=re.IGNORECASE
        ).strip()
    ...
    class CustomTorrent(torf.Torrent):
        ...
        def __init__(self, meta, *args, **kwargs):
            ...
            if "max_piece_size" in meta and meta["max_piece_size"]:
                try:
                    max_piece_size_mib = (
                        int(meta["max_piece_size"]) * 1024 * 1024
                    )  # Convert MiB to bytes
                    self.piece_size_max = min(
                        max_piece_size_mib, torf.Torrent.piece_size_max
                    )
                except ValueError:
                    self.piece_size_max = (
            ...
            piece_size = self.calculate_piece_size(
                total_size, self.piece_size_min, self.piece_size_max, self.files
            )
        ...
        def piece_size(self, value):
            ...
            self.metainfo["info"][
                "piece length"
            ] = value  # Ensure 'piece length' is set

        @classmethod
        def calculate_piece_size(cls, total_size, min_size, max_size, files):
            our_min_size = 16384
            our_max_size = (
                max_size if max_size else 67108864
            )  # Default to 64 MiB if max_size is None
            piece_size = 8388608  # Start with 8 MiB
            num_pieces = math.ceil(total_size / piece_size)
            torrent_file_size = (
                20 + (num_pieces * 20) + cls._calculate_pathname_bytes(files)
            )  # Approximate .torrent size

            # Adjust the piece size to fit within the constraints
            while not (
                1000 <= num_pieces <= 2000 and torrent_file_size <= 102400
            ):  # 100 KiB .torrent size limit
                if num_pieces < 1000:
                    piece_size //= 2
                    ...
                    if piece_size > our_max_size:
                        cli_ui.warning(
                            f"Warning: Piece size exceeded 2000 pieces and .torrent will be approximately {torrent_file_size / 1024:.2f} KiB! Using ({num_pieces}) pieces."
                        )
        ...
        def validate_piece_size(self):
            ...
            self.metainfo["info"][
                "piece length"
            ] = self.piece_size  # Ensure 'piece length' is set
    ...
    def create_torrent(self, meta, path, output_filename):
        ...
        if meta["isdir"]:
            if meta["keep_folder"]:
                ...
                for file in globs:
                    if (
                        not file.lower().endswith("sample.mkv")
    ...
    def create_random_torrents(self, base_dir, uuid, num, path):
        ...
        for i in range(1, int(num) + 1):
            ...
            Torrent.copy(new_torrent).write(
                f"{base_dir}/tmp/{uuid}/[RAND-{i}]{manual_name}.torrent", overwrite=True
            )
    ...
    def create_base_from_existing_torrent(self, torrentpath, base_dir, uuid):
        if os.path.exists(torrentpath):
            ...
            Torrent.copy(base_torrent).write(
                f"{base_dir}/tmp/{uuid}/BASE.torrent", overwrite=True
            )
    ...
    def upload_screens(
        ...
        if (
            len(existing_images) >= total_screens
            and not retry_mode
            and img_host == initial_img_host
        ):
            console.print(
                f"[yellow]Skipping upload because images are already uploaded to {img_host}. Existing images: {len(existing_images)}, Required: {total_screens}"
            )
        ...
        if img_host == "imgbox":
            ...
            image_list = asyncio.run(
                self.imgbox_upload(f"{meta['base_dir']}/tmp/{meta['uuid']}", image_glob)
            )
            if not image_list:
                ...
                if not img_host:
                    console.print(
                        "[red]All image hosts failed. Unable to complete uploads."
                    )
                    return image_list, i
            else:
            ...
            with Progress(
                ...
                while True:
                    upload_task = progress.add_task(
                        f"[green]Uploading Screens to {img_host}...",
                        total=len(image_glob[-screens:]),
                    )
                    ...
                    for image in image_glob[-screens:]:
                        try:
                            ...
                            if img_host == "ptpimg":
                                ...
                                img_url = (
                                    f"https://ptpimg.me/{ptpimg_code}.{ptpimg_ext}"
                                )
                                raw_url = img_url
                                web_url = img_url
                            elif img_host == "imgbb":
                                ...
                                if response.get("status_code") != 200:
                                    console.print(
                                        "[yellow]PT Screens failed, trying next image host"
                                    )
                                    break
                                img_url = response["data"]["image"]["url"]
                                raw_url = img_url
                                ...
                                if response.status_code != 200:
                                    console.print(
                                        "[yellow]Pixhost failed, trying next image host"
                                    )
                                    break
                                response = response.json()
                                raw_url = (
                                ...
                                if response.get("status_code") != 200:
                                    console.print(
                                        "[yellow]Lensdump failed, trying next image host"
                                    )
                                    break
                                img_url = response["data"]["image"]["url"]
                                raw_url = img_url
                                ...
                                console.print(
                                    f"[red]Unsupported image host: {img_host}"
                                )
                                break

                            # Update progress bar and print the result on the same line
                            progress.console.print(
                                f"[cyan]Uploaded image {i + 1}/{total_screens}: {raw_url}",
                                end="\r",
                            )
                            ...
                            console.print(
                                f"[yellow]Failed to upload {image} to {img_host}. Exception: {str(e)}"
                            )
                            break
    ...
    async def imgbox_upload(self, chdir, image_glob):
        ...
        with Progress(
            ...
            async with pyimgbox.Gallery(
                thumb_width=350, square_thumbs=False
            ) as gallery:
                async for submission in gallery.add(image_glob):
                    if not submission["success"]:
                        console.print(
                            f"[red]There was an error uploading to imgbox: [yellow]{submission['error']}[/yellow][/red]"
                        )
                        return []
                    else:
                        image_dict = {}
        ...
        if meta["category"] == "TV":
            ...
            if meta["anime"] is False:
                try:
                    ...
                    try:
                        guess_date = (
                            meta.get("manual_date", guessit(video)["date"])
                            if meta.get("manual_date")
                            else guessit(video)["date"]
                        )
                        season_int, episode_int = self.daily_to_tmdb_season_episode(
                            meta.get("tmdb"), guess_date
                        )
                ...
                romaji, mal_id, eng_title, seasonYear, anilist_episodes = (
                    self.get_romaji(parsed["anime_title"], meta.get("mal", None))
                )
                if mal_id:
                    meta["mal_id"] = mal_id
                if meta.get("tmdb_manual", None) is None:
                    ...
                    meta = await self.get_tmdb_id(
                        guessit(
                            parsed["anime_title"], {"excludes": ["country", "language"]}
                        )["title"],
                        year,
                        meta,
                        meta["category"],
                ...
                if len(filelist) == 1:
                    try:
                        episodes = parsed.get(
                            "episode_number", guessit(video).get("episode", "1")
                        )
                        ...
                        if isinstance(episodes, list):
                            ...
                            episode = "".join(
                                [f"E{str(int(item)).zfill(2)}" for item in episodes]
                            )
                        else:
                ...
                try:
                    if meta.get("season_int"):
                        ...
                        season = parsed.get(
                            "anime_season", guessit(video).get("season", "1")
                        )
                                        ...
                                        if lang == "jp":
                                            for name in names:
                                                ...
                                                diff = SequenceMatcher(
                                                    None, romaji_check, name_check
                                                ).ratio()
                                                if (
                                                    romaji_check in name_check
                                                    and diff >= difference
                                                ):
                                                    season_int = (
                                                        int(season_num)
            ...
            if meta.get("manual_season", None) is None:
                ...
                meta["season"] = (
                    f"S{meta['manual_season'].lower().replace('s', '').zfill(2)}"
                )
            if meta.get("manual_episode", None) is None:
                ...
                meta["episode"] = (
                    f"E{meta['manual_episode'].lower().replace('e', '').zfill(2)}"
                )
                meta["tv_pack"] = 0
            ...
            meta["episode_title_storage"] = guessit(video, {"excludes": "part"}).get(
                "episode_title", ""
            )
    ...
    def get_service(
        ...
        video_name = re.sub(
            r"[.()]", " ", video.replace(tag, "").replace(guess_title, "")
        )
        if "DTS-HD MA" in audio:
            video_name = video_name.replace("DTS-HD.MA.", "").replace("DTS-HD MA ", "")
        for key, value in services.items():
            if (" " + key + " ") in video_name and key not in guessit(
                video, {"excludes": ["country", "language"]}
            ).get("title", ""):
                service = value
            elif key == service:
                service = value
    ...
    def is_anon(self, anon_in):
        ...
        if anon.lower() == "true":
            console.print(
                "[bold red]Global ANON has been removed in favor of per-tracker settings. Please update your config accordingly."
            )
            time.sleep(10)
        if anon_in is True:
            anon_out = 1
        ...
        with open(
            ...
            if (desclink, descfile, meta["desc"]) == (None, None, None):
                if (
                    ...
                    == "true"
                    and desc_source in ["PTP", None]
                ):
                    if meta.get("skip_gen_desc", False):
                        console.print(
                            "[cyan]Something went wrong with PTP description."
                        )
                        return meta

                    ptp = PTP(config=self.config)
                    ptp_desc, imagelist = await ptp.get_ptp_description(
                        meta["ptp"], meta["is_disc"]
                    )
                ...
                if (
                    not ptp_desc
                    and clean_text(meta.get("blu_desc", ""))
                    and desc_source in ["BLU", None]
                ):
                ...
                if (
                    not ptp_desc
                    and clean_text(meta.get("lst_desc", ""))
                    and desc_source in ["LST", None]
                ):
                ...
                if (
                    not ptp_desc
                    and clean_text(meta.get("aither_desc", ""))
                    and desc_source in ["AITHER", None]
                ):
                ...
                if (
                    not ptp_desc
                    and clean_text(meta.get("oe_desc", ""))
                    and desc_source in ["OE", None]
                ):
                ...
                if (
                    not ptp_desc
                    and clean_text(meta.get("tike_desc", ""))
                    and desc_source in ["TIK", None]
                ):
            ...
            if meta.get("desc_template"):
                ...
                try:
                    with open(
                        f"{meta['base_dir']}/data/templates/{meta['desc_template']}.txt",
                        "r",
                    ) as f:
                        ...
                        if clean_text(template_desc):
                            ...
                            console.print(
                                f"[INFO] Description from template '{meta['desc_template']}' used."
                            )
                except FileNotFoundError:
                    console.print(
                        f"[ERROR] Template '{meta['desc_template']}' not found."
                    )
    ...
    async def package(self, meta):
        ...
        with open(
            f"{meta['base_dir']}/tmp/{meta['uuid']}/GENERIC_INFO.txt",
            "w",
            encoding="utf-8",
        ) as generic:
            ...
            if meta.get("poster", None) not in ["", None] and not os.path.exists(
                poster_img
            ):
                if meta.get("rehosted_poster", None) is None:
                    ...
                    if r.status_code == 200:
                        ...
                        poster, dummy = self.upload_screens(
                            meta, 1, 1, 0, 1, [poster_img], {}
                        )
                        poster = poster[0]
                        generic.write(
                            f"TMDB Poster: {poster.get('raw_url', poster.get('img_url'))}\n"
                        )
                        meta["rehosted_poster"] = poster.get(
                            "raw_url", poster.get("img_url")
                        )
                        with open(
                            f"{meta['base_dir']}/tmp/{meta['uuid']}/meta.json", "w"
                        ) as metafile:
        ...
        try:
            if os.path.exists(f"{meta['base_dir']}/tmp/{meta['uuid']}/BASE.torrent"):
                ...
                manual_name = re.sub(
                    r"[^0-9a-zA-Z\[\]\'\-]+", ".", os.path.basename(meta["path"])
                )
                Torrent.copy(base_torrent).write(
                    f"{meta['base_dir']}/tmp/{meta['uuid']}/{manual_name}.torrent",
                    overwrite=True,
                )
            ...
            filebrowser = (
                self.config["TRACKERS"].get("MANUAL", {}).get("filebrowser", None)
            )
            shutil.make_archive(
                archive, "tar", f"{meta['base_dir']}/tmp/{meta['uuid']}"
            )
            if filebrowser is not None:
                url = "/".join(
                    s.strip("/") for s in (filebrowser, f"/tmp/{meta['uuid']}")
                )
                ...
                response = requests.post(
                    "https://uguu.se/upload.php", files=files
                ).json()
                if meta["debug"]:
    ...
    async def get_imdb_aka(self, imdb_id):
        ...
        aka = (
            result.get("original title", result.get("localized title", ""))
            .replace(" - IMDb", "")
            .replace("\u00ae", "")
        )
        if aka != "":
    ...
    def get_tmdb_imdb_from_mediainfo(
        self, mediainfo, category, is_disc, tmdbid, imdbid
    ):
        if not is_disc:
            if mediainfo["media"]["track"][0].get("extra"):
                ...
                for each in extra:
                    if each.lower().startswith("tmdb"):
                        parser = Args(config=self.config)
                        category, tmdbid = parser.parse_tmdb_id(
                            id=extra[each], category=category
                        )
    ...
    def daily_to_tmdb_season_episode(self, tmdbid, date):
        ...
        for each in season_info:
            ...
            console.print(
                f"[yellow]Unable to map the date ([bold yellow]{str(date)}[/bold yellow]) to a Season/Episode number"
            )
    ...
    async def get_imdb_info(self, imdbID, meta):
        ...
        if int(str(imdbID).replace("tt", "")) != 0:
            ...
            imdb_info["aka"] = info.get(
                "original title", info.get("localized title", imdb_info["title"])
            ).replace(" - IMDb", "")
            imdb_info["type"] = info.get("kind")
            imdb_info["imdbID"] = info.get("imdbID")
            imdb_info["runtime"] = info.get("runtimes", ["0"])[0]
            imdb_info["cover"] = info.get("full-size cover url", "").replace(
