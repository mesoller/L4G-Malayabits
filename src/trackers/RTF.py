...
class RTF:
    ...
    def __init__(self, config):
        ...
        self.forum_link = (
            "https://retroflix.club/forums.php?action=viewtopic&topicid=3619"
        )
        self.banned_groups = []
        pass
    ...
    async def upload(self, meta, disctype):
        ...
        json_data = {
            ...
            + (
                meta["imdb_id"]
                if str(meta["imdb_id"]).startswith("tt")
                else "tt" + meta["imdb_id"]
            )
            + "/",
        ...
        if datetime.date.today().year - meta["year"] <= 9:
            console.print(
                "[red]ERROR: Not uploading!\nMust be older than 10 Years as per rules"
            )
            return

        if meta["debug"] is False:
            response = requests.post(
                url=self.upload_url, json=json_data, headers=headers
            )
            try:
        ...
        try:
            async with httpx.AsyncClient() as client:
                response = await client.post(
                    "https://retroflix.club/api/login", headers=headers, json=json_data
                )
            ...
            if response.status_code == 201:
                ...
                if token:
                    console.print(
                        "[bold green]Saving and using New API key generated for this upload"
                    )
                    ...
                    console.print(
                        f"[bold green]API Key successfully saved to {config_path}"
                    )
                else:
                    console.print("[bold red]API response does not contain a token.")
            else:
                console.print(
                    f"[bold red]Error getting new API key: {response.status_code}, please check username and password in the config."
                )

        except httpx.RequestError as e:
            console.print(
                f"[bold red]An error occurred while requesting the API: {str(e)}"
            )
