...
class TTG:
    ...
    def __init__(self, config):
        ...
        self.passkey = (
            str(config["TRACKERS"]["TTG"].get("announce_url", ""))
            .strip()
            .split("/")[-1]
        )
    ...
    async def get_type_id(self, meta):
        ...
        if "documentary" in meta.get("genres", "").lower().replace(" ", "").replace(
            "-", ""
        ) or "documentary" in meta.get("keywords", "").lower().replace(" ", "").replace(
            "-", ""
        ):
            if meta["resolution"].startswith("720"):
                type_id = 62  # 720p
        ...
        if "animation" in meta.get("genres", "").lower().replace(" ", "").replace(
            "-", ""
        ) or "animation" in meta.get("keywords", "").lower().replace(" ", "").replace(
            "-", ""
        ):
            if meta.get("sd", 1) == 0:
                type_id = 58
            ...
            if meta["debug"]:
                ...
                with requests.Session() as session:
                    cookiefile = os.path.abspath(
                        f"{meta['base_dir']}/data/cookies/TTG.pkl"
                    )
                    ...
                    if up.url.startswith("https://totheglory.im/details.php?id="):
                        console.print(
                            f"[green]Uploaded to: [yellow]{up.url}[/yellow][/green]"
                        )
    ...
    async def search_existing(self, meta, disctype):
        ...
        with requests.Session() as session:
            ...
            search_url = (
                f"https://totheglory.im/browse.php?search_field= {imdb} {res_type}"
            )
    ...
    async def login(self, cookiefile):
        ...
        with requests.Session() as session:
            ...
            if response.url.endswith("2fa.php"):
                ...
                auth_token = soup.find("input", {"name": "authenticity_token"}).get(
                    "value"
                )
                two_factor_data = {
    ...
    async def download_new_torrent(self, id, torrent_path):
        ...
        if r.status_code == 200:
            ...
            console.print(
                "[red]There was an issue downloading the new .torrent from TTG"
            )
