...
class BLU:
    ...
    async def upload(self, meta, disctype):
        ...
        await common.unit3d_edit_desc(
            meta, self.tracker, self.signature, comparison=True, desc_header=desc_header
        )
        ...
        if meta["bdinfo"] is not None:
            ...
            bd_dump = None
        desc = open(
            f"{meta['base_dir']}/tmp/{meta['uuid']}/[BLU]DESCRIPTION.txt",
            "r",
            encoding="utf-8",
        ).read()
        open_torrent = open(
            f"{meta['base_dir']}/tmp/{meta['uuid']}/[BLU]{meta['clean_name']}.torrent",
            "rb",
        )
        files = {
    ...
    async def get_cat_id(self, category_name, edition):
        category_id = {"MOVIE": "1", "TV": "2", "FANRES": "3"}.get(category_name, "0")
    ...
    async def derived_dv_layer(self, meta):
        ...
        console.print(
            "[bold yellow]Generating the required description addition for Derived DV Layers. Please respond appropriately."
        )
        ask_comp = True
        if meta["type"] == "WEBDL":
            if cli_ui.ask_yes_no(
                "Is the DV Layer sourced from the same service as the video?"
            ):
                ask_comp = False
        ...
        if ask_comp:
            while desc_header == "":
                desc_input = cli_ui.ask_string(
                    "Please provide comparisons between HDR masters. (link or bbcode)",
                    default="",
                )
    ...
    async def search_existing(self, meta, disctype):
        ...
        params = {
            ...
            "categories[]": await self.get_cat_id(
                meta["category"], meta.get("edition", "")
            ),
