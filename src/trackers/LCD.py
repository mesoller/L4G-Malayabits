...
class LCD:
    ...
    async def upload(self, meta, disctype):
        ...
        if meta["bdinfo"] is not None:
            ...
            bd_dump = None
        desc = open(
            f"{meta['base_dir']}/tmp/{meta['uuid']}/[LCD]DESCRIPTION.txt",
            "r",
            encoding="utf-8",
        ).read()
        open_torrent = open(
            f"{meta['base_dir']}/tmp/{meta['uuid']}/[LCD]{meta['clean_name']}.torrent",
            "rb",
        )
        files = {
    ...
    async def get_cat_id(self, category_name, edition, meta):
        category_id = {"MOVIE": "1", "TV": "2", "ANIMES": "6"}.get(category_name, "0")
