...
class COMMON:
    ...
    async def edit_torrent(self, meta, tracker, source_flag, torrent_filename="BASE"):
        if os.path.exists(
            f"{meta['base_dir']}/tmp/{meta['uuid']}/{torrent_filename}.torrent"
        ):
    ...
    async def unit3d_edit_desc(
        self, meta, tracker, signature, comparison=False, desc_header=""
    ):
        base = open(
        ...
        with open(
            f"{meta['base_dir']}/tmp/{meta['uuid']}/[{tracker}]DESCRIPTION.txt",
            "w",
            encoding="utf8",
        ) as descfile:
            if desc_header != "":
    ...
    async def prompt_user_for_id_selection(
        self, tmdb=None, imdb=None, tvdb=None, filename=None, tracker_name=None
    ):
        if not tracker_name:
        ...
        if imdb:
            ...
            console.print(
                f"[cyan]Found IMDb ID: https://www.imdb.com/title/tt{imdb}[/cyan]"
            )
        ...
        selection = (
            input(f"Do you want to use these IDs from {tracker_name}? (Y/n): ")
            .strip()
            .lower()
        )
        try:
    ...
    async def unit3d_torrent_info(
        self, tracker, torrent_url, search_url, meta, id=None, file_name=None
    ):
        tmdb = imdb = tvdb = description = category = infohash = mal = files = (
            None  # noqa F841
        )
        imagelist = []
        ...
        try:
            ...
            if data:
                ...
                if id and not data:
                    ...
                    console.print(
                        f"[green]Valid IDs found: TMDb: {tmdb}, IMDb: {imdb}, TVDb: {tvdb}[/green]"
                    )
            ...
            if tmdb or imdb or tvdb:
                if not id:
                    ...
                    try:
                        if not await self.prompt_user_for_id_selection(
                            tmdb, imdb, tvdb, file_name
                        ):
                            console.print(
            ...
            if description:
                ...
                console.print(
                    f"[blue]Extracted description: [yellow]{description}", markup=False
                )
                ...
                if not (
                    ...
                    or meta.get("oe")
                    or meta.get("tik")
                ) or meta.get("unattended"):
                    console.print(
                        "[cyan]Do you want to edit, discard or keep the description?[/cyan]"
                    )
                    edit_choice = input(
                        "[cyan]Enter 'e' to edit, 'd' to discard, or press Enter to keep it as is: [/cyan]"
                    )
            ...
            console.print(
                f"[yellow]Invalid Response from {tracker} API. Error: {str(e)}[/yellow]"
            )
            return None, None, None, None, None, None, None, None, None
    ...
    async def ptgen(self, meta, ptgen_site="", ptgen_retry=3):
        ...
        if int(meta.get("imdb_id", "0")) != 0:
            ...
            try:
                ...
                params["url"] = console.input(
                    "[red]Please enter [yellow]Douban[/yellow] link: "
                )
        else:
    ...
    async def filter_dupes(self, dupes, meta):
        ...
        for each in dupes:
            ...
            search_matches = [
                {"if": {"REMUX", "WEBDL", "WEBRip", "HDTV"}, "in": meta["type"]}
            ]
            for s in search_combos:
                if s.get("search_for") not in (None, ""):
                    if any(
                        re.search(x, s["search"], flags=re.IGNORECASE)
                        for x in s["search_for"]
                    ):
                        remove_set.update(s["update"])
                if s.get("search_not") not in (None, ""):
                    if not any(
                        re.search(x, s["search"], flags=re.IGNORECASE)
                        for x in s["search_not"]
                    ):
                        remove_set.update(s["update"])
            ...
            for x in remove_set:
                if not x.startswith("!"):
                    ...
                    if re.search(x.replace("!", "", 1), search, flags=re.I) not in (
                        None,
                        False,
                    ):
                        allow = False
