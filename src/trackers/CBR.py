...
class CBR:
    ...
    async def upload(self, meta, disctype):
        ...
        if meta["bdinfo"] is not None:
            ...
            bd_dump = None
        desc = open(
            f"{meta['base_dir']}/tmp/{meta['uuid']}/[CBR]DESCRIPTION.txt",
            "r",
            encoding="utf-8",
        ).read()
        open_torrent = open(
            f"{meta['base_dir']}/tmp/{meta['uuid']}/[CBR]{meta['clean_name']}.torrent",
            "rb",
        )
        files = {
            "torrent": ("placeholder.torrent", open_torrent, "application/x-bittorrent")
        }
        data = {
            "name": name,
            "description": desc,
    ...
    async def get_cat_id(self, category_name, edition, meta):
        category_id = {"MOVIE": "1", "TV": "2", "ANIMES": "4"}.get(category_name, "0")
    ...
    async def search_existing(self, meta, disctype):
        ...
        params = {
            ...
            "categories[]": await self.get_cat_id(
                meta["category"], meta.get("edition", ""), meta
            ),
        ...
        try:
            ...
            console.print(
                "[bold red]Não foi possivel buscar no tracker torrents duplicados. O tracker está offline ou sua api está incorreta"
            )
    ...
    async def edit_name(self, meta):
        ...
        name = (
            ...
            .replace(".mp4", "")
            .replace(".", " ")
            .replace("DDP2 0", "DDP2.0")
            .replace("DDP5 1", "DDP5.1")
            .replace("H 264", "H.264")
            .replace("H 265", "H.265")
            .replace("DD+7 1", "DDP7.1")
            .replace("AAC2 0", "AAC2.0")
            .replace("DD5 1", "DD5.1")
            .replace("DD2 0", "DD2.0")
            .replace("TrueHD 7 1", "TrueHD 7.1")
            .replace("DTS-HD MA 7 1", "DTS-HD MA 7.1")
            .replace("DTS-HD MA 5 1", "DTS-HD MA 5.1")
            .replace("TrueHD 5 1", "TrueHD 5.1")
            .replace("DTS-X 7 1", "DTS-X 7.1")
            .replace("DTS-X 5 1", "DTS-X 5.1")
            .replace("FLAC 2 0", "FLAC 2.0")
            .replace("FLAC 5 1", "FLAC 5.1")
            .replace("DD1 0", "DD1.0")
            .replace("DTS ES 5 1", "DTS ES 5.1")
            .replace("DTS5 1", "DTS 5.1")
            .replace("AAC1 0", "AAC1.0")
            .replace("DD+5 1", "DDP5.1")
            .replace("DD+2 0", "DDP2.0")
            .replace("DD+1 0", "DDP1.0")
        )
