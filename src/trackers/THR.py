...
class THR:
    ...
    async def upload(self, session, meta, disctype):
        ...
        if meta.get("unattended", False) is False:
            ...
            if thr_confirm is not True:
                thr_name_manually = cli_ui.ask_string(
        ...
        files = {"tfile": (f"{torrent_name}.torrent", tfile)}
        payload = {
            "name": thr_name,
            "descr": desc,
            "type": cat_id,
        ...
        if meta["debug"] is False:
            ...
            thr_upload_prompt = cli_ui.ask_yes_no(
                "send to takeupload.php?", default=False
            )
    ...
    def get_subtitles(self, meta):
        ...
        if meta.get("is_disc", "") != "BDMV":
            with open(
                f"{meta.get('base_dir')}/tmp/{meta.get('uuid')}/MediaInfo.json",
                "r",
                encoding="utf-8",
            ) as f:
                mi = json.load(f)
    ...
    async def edit_torrent(self, meta):
        if os.path.exists(f"{meta['base_dir']}/tmp/{meta['uuid']}/BASE.torrent"):
            ...
            THR_torrent.metainfo["info"][
                "source"
            ] = "[https://www.torrenthr.org] TorrentHR.org"
    ...
    async def edit_desc(self, meta):
        ...
        with open(
            ...
            if meta.get("is_disc", "") == "BDMV":
                ...
                data = {
                    "content": open(
                        f"{meta['base_dir']}/tmp/{meta['uuid']}/MEDIAINFO.txt", "r"
                    ).read(),
                ...
                try:
                    ...
                    console.print(
                        "[bold red]Error parsing pronfo response, using THR parser instead"
                    )
                    if meta["debug"]:
    ...
    def search_existing(self, session, imdb_id, disctype):
        ...
        search_url = (
            f"https://www.torrenthr.org/browse.php?search={imdb_id}&blah=2&incldead=1"
        )
    ...
    def login(self, session):
        ...
        payload = {"username": self.username, "password": self.password, "ssl": "yes"}
        headers = {
            "User-Agent": f"Upload Assistant/2.1 ({platform.system()} {platform.release()})"
        }
