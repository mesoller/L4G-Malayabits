...
class ANT:
    ...
    async def upload(self, meta, disctype):
        ...
        num_pieces, torrent_file_size = calculate_pieces_and_file_size(
            total_size, total_pathname_bytes, torrent.piece_size
        )
        ...
        if not (1000 <= num_pieces <= 2000) or torrent_file_size > 102400:
            console.print(
                f"[yellow]Existing .torrent is outside of ANT preferred constraints with {num_pieces} pieces and is approximately {torrent_file_size_kib:.2f} KiB."
            )
            regenerate = cli_ui.ask_yes_no(
                "Do you wish to regenerate the torrent?", default=True
            )

            if regenerate:
                console.print(
                    "[yellow]Regenerating torrent to fit within 1000-2000 pieces and 100 KiB .torrent size limit needed for ANT."
                )
                ...
                console.print(
                    "[green]Using the existing torrent despite not meeting the preferred constraints."
                )
        else:
