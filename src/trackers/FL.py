...
class FL:
    ...
    async def edit_name(self, meta):
        ...
        if meta.get("imdb_info"):
            ...
            if (
                meta["year"] != meta.get("imdb_info", {}).get("year", meta["year"])
        ...
        if (
            ...
            fl_name = fl_name.replace(
                meta["episode"], f"{meta['episode']} {meta['episode_title_storage']}"
            )
    ...
    async def upload(self, meta, disctype):
        ...
        if meta.get("unattended", False) is False:
            ...
            if fl_confirm is not True:
                fl_name_manually = cli_ui.ask_string(
                    "Please enter a proper name", default=""
                )
                if fl_name_manually == "":
        ...
        with open(torrent_path, "rb") as torrentFile:
            ...
            if (
                self.uploader_name not in ("", None)
                and bool(
                    str2bool(
            ...
            if meta["debug"]:
                ...
                with requests.Session() as session:
                    cookiefile = os.path.abspath(
                        f"{meta['base_dir']}/data/cookies/FL.pkl"
                    )
                    ...
                    match = re.match(
                        r".*?filelist\.io/details\.php\?id=(\d+)&uploaded=(\d+)", up.url
                    )
                    if match:
    ...
    async def search_existing(self, meta, disctype):
        ...
        with requests.Session() as session:
            ...
            for each in find:
                if (
                    each["href"].startswith("details.php?id=")
                    and "&" not in each["href"]
                ):
    ...
    async def login(self, cookiefile):
        with requests.Session() as session:
            ...
            if response.text.find("Logout") != -1:
                ...
                console.print(
                    "[bold red]Something went wrong while trying to log into FL"
                )
    ...
    async def download_new_torrent(self, session, id, torrent_path):
        ...
        if r.status_code == 200:
            ...
            console.print(
                "[red]There was an issue downloading the new .torrent from FL"
            )
    ...
    async def edit_desc(self, meta):
        ...
        with open(
            ...
            if meta["is_disc"] != "BDMV":
                    ...
                    files.append(
                        (
                            ...
                            (
                                ...
                                open(
                                    f"{meta['base_dir']}/tmp/{meta['uuid']}/{screen}",
                                    "rb",
                                ),
                                "image/png",
                            ),
                ...
                final_desc = open(
                    f"{meta['base_dir']}/tmp/{meta['uuid']}/BD_SUMMARY_EXT.txt",
                    "r",
                    encoding="utf-8",
                ).read()
                if final_desc.strip() != "":  # Use BD_SUMMARY_EXT and bbcode format it
                    ...
                    final_desc = (
                        ...
                        .replace(
                            "SUBTITLES:", "[b][color=#FF0000]SUBTITLES:[/color][/b]"
                        )
                    )
