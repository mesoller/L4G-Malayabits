...
class HUNO:
    ...
    def __init__(self, config):
        ...
        self.banned_groups = [
            "4K4U, Bearfish, BiTOR, BONE, D3FiL3R, d3g, DTR, ELiTE, EVO, eztv, EzzRips, FGT, HashMiner, HETeam, HEVCBay, HiQVE, HR-DR, iFT, ION265, iVy, JATT, Joy, LAMA, m3th, MeGusta, MRN, Musafirboy, OEPlus, Pahe.in, PHOCiS, PSA, RARBG, RMTeam, ShieldBearer, SiQ, TBD, Telly, TSP, VXT, WKS, YAWNiX, YIFY, YTS"
        ]
        pass
    ...
    async def upload(self, meta, disctype):
        ...
        if (
            meta["anon"] == 0
            and bool(str2bool(self.config["TRACKERS"]["HUNO"].get("anon", "False")))
            is False
        ):
            anon = 0
        else:
        ...
        if meta["video_codec"] != "HEVC" and (
            meta["type"] == "ENCODE" or meta["type"] == "WEBRIP"
        ):
        ...
        if meta["bdinfo"] is not None:
            ...
            bd_dump = None
        desc = open(
            f"{meta['base_dir']}/tmp/{meta['uuid']}/[HUNO]DESCRIPTION.txt",
            "r",
            encoding="utf-8",
        ).read()
        open_torrent = open(
            f"{meta['base_dir']}/tmp/{meta['uuid']}/[HUNO]{meta['clean_name']}.torrent",
            "rb",
        )
        ...
        if "internal" in tracker_config:
            if (
                tracker_config["internal"]
                and meta["tag"]
                and meta["tag"][1:] in tracker_config.get("internal_groups", [])
            ):
                data["internal"] = 1
            else:
                data["internal"] = 0
        ...
        params = {"api_token": tracker_config["api_key"].strip()}
    ...
    def get_audio(self, meta):
        ...
        if dual:
            ...
            language = next(
                x for x in meta["mediainfo"]["media"]["track"] if x["@type"] == "Audio"
            ).get("Language_String", "English")
    ...
    async def get_name(self, meta):
        ...
        scale = (
            "DS4K"
            if "DS4K" in basename.upper()
            else "RM4K" if "RM4K" in basename.upper() else ""
        )
