...
class BHD:
                ...
                if len(discs) >= 2:
                    for each in discs[1:]:
                        if each["type"] == "BDMV":
                            ...
                            desc.write(f"{each['name']}:\n")
                            desc.write(
                                f"[spoiler={os.path.basename(each['largest_evo'])}][code][{each['evo_mi']}[/code][/spoiler]\n"
                            )
                            desc.write("\n")
    ...
    async def search_existing(self, meta, disctype):
        ...
        try:
            ...
            if response.get("status_code") == 1:
                for each in response["results"]:
                    ...
                    difference = SequenceMatcher(
                        None, meta["clean_name"].replace("DD+", "DDP"), result
                    ).ratio()
                    if difference >= 0.05:
                        dupes.append(result)
            else:
            ...
            console.print(
                "[bold red]Unable to search for existing torrents on site. Most likely the site is down."
            )
            await asyncio.sleep(5)
