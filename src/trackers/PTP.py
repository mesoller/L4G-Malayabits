...
class PTP:
    ...
    def __init__(self, config):
        ...
        self.web_source = str2bool(
            str(config["TRACKERS"]["PTP"].get("add_web_source_to_desc", True))
        )
        self.user_agent = (
            f"Upload Assistant/2.1 ({platform.system()} {platform.release()})"
        )
        self.banned_groups = [
            "aXXo",
            "BMDru",
    ...
    async def get_ptp_id_imdb(self, search_term, search_file_folder, meta):
        ...
        console.print(
            f"[green]Searching PTP for: [bold yellow]{filename}[/bold yellow]"
        )
        ...
        try:
            if response.status_code == 200:
                            ...
                            for torrent in movie["Torrents"]:
                                ...
                                for file in torrent["FileList"]:
                                    if file.get("Path") == filename:
                                        ...
                                        dummy, ptp_torrent_hash, *_ = (
                                            await self.get_imdb_from_torrent_id(
                                                ptp_torrent_id
                                            )
                                        )
                                        console.print(
                                            f"[bold green]Matched release with PTP ID: [yellow]{ptp_torrent_id}[/yellow][/bold green]"
                                        )

                                        # Call get_torrent_info and print the results
                                        tinfo = await self.get_torrent_info(
                                            imdb_id, meta
                                        )
                                        console.print(
                                            f"[cyan]Torrent Info: {tinfo}[/cyan]"
                                        )
                ...
                console.print(
                    f"[yellow]Could not find any release matching [bold yellow]{filename}[/bold yellow] on PTP"
                )
    ...
    async def get_ptp_description(self, ptp_torrent_id, meta, is_disc):
        ...
        url = "https://passthepopcorn.me/torrents.php"
        console.print(
            f"[yellow]Requesting description from {url} with ID {ptp_torrent_id}"
        )
        response = requests.get(url, params=params, headers=headers)
        await asyncio.sleep(1)
        ...
        console.print(
            f"[cyan]Description after cleaning:[yellow]\n{desc[:1000]}..."
        )  # Show first 1000 characters for brevity
    ...
    async def get_group_by_imdb(self, imdb):
        ...
        try:
            ...
            if response.get("Page") == "Browse":  # No Releases on Site with ID
                ...
                console.print(
                    f"[green]Matched IMDb: [yellow]tt{imdb}[/yellow] to Group ID: [yellow]{groupID}[/yellow][/green]"
                )
                console.print(
                    f"[green]Title: [yellow]{response.get('Name')}[/yellow] ([yellow]{response.get('Year')}[/yellow])"
                )
                return groupID
        except Exception:
            console.print("[red]An error has occured trying to find a group ID")
            console.print(
                "[red]Please check that the site is online and your ApiUser/ApiKey values are correct"
            )
            return None
    ...
    async def get_tags(self, check_against):
        ...
        for each in ptp_tags:
            if any(
                each.replace(".", "") in x
                for x in check_against.lower().replace(" ", "").replace("-", "")
            ):
                tags.append(each)
        return tags
    ...
    def get_type(self, imdb_info, meta):
        ...
        if imdb_info["type"] is not None:
            ...
            if imdbType in ("movie", "tv movie"):
                if (
                    int(imdb_info.get("runtime", "60")) >= 45
                    or int(imdb_info.get("runtime", "60")) == 0
                ):
            ...
            if tmdb_type == "movie":
                if (
                    int(meta.get("runtime", 60)) >= 45
                    or int(meta.get("runtime", 60)) == 0
                ):
                    ptpType = "Feature Film"
                else:
                    ptpType = "Short Film"
        ...
        if ptpType is None:
            if meta.get("mode", "discord") == "cli":
                ...
                ptpType = cli_ui.ask_choice(
                    "Select the proper type", choices=ptpTypeList
                )
        return ptpType
    ...
    def get_resolution(self, meta):
        ...
        if (res == "OTHER" and meta["is_disc"] != "BDMV") or (
            meta["sd"] == 1 and meta["type"] == "WEBDL"
        ):
    ...
    def get_trumpable(self, sub_langs):
        ...
        opts = cli_ui.select_choices(
            "English subtitles not found. Please select any/all applicable options:",
            ...
            for t, v in trumpable_values.items():
                if t in "".join(opts):
                    if v is None:
                        ...
                        trumpable.append(14)
                        hc_sub_langs = cli_ui.ask_string(
                            "Enter language code for HC Subtitle languages"
                        )
                ...
                for i in range(len(discs)):
                    ...
                    if each["type"] == "BDMV":
                        ...
                        if i == 0:
                            ...
                            images, dummy = prep.upload_screens(
                                meta, 2, 1, 0, 2, new_screens, {}
                            )
                    ...
                    if each["type"] == "DVD":
                        ...
                        if i == 0:
                            ...
                            ds = multiprocessing.Process(
                                target=prep.dvd_screenshots, args=(meta, i, 2)
                            )
                            ds.start()
                            while ds.is_alive() is True:
                                await asyncio.sleep(1)
            ...
            # For non-discs
                ...
                for i in range(len(meta["filelist"])):
                    ...
                    if i == 0:
                        ...
                        if (
                            meta["type"] == "WEBDL"
                            and meta.get("service_longname", "") != ""
                            and meta.get("description", None) is None
                            and self.web_source is True
                        ):
                            desc.write(
                                f"[quote][align=center]This release is sourced from {meta['service_longname']}[/align][/quote]"
                            )
                        mi_dump = open(
                        ...
                        with open(
                            f"{meta['base_dir']}/tmp/{meta['uuid']}/TEMP_PTP_MEDIAINFO.txt",
                            "w",
                            newline="",
                            encoding="utf-8",
                        ) as f:
            ...
            if loggedIn is True:
                ...
                try:
                    ...
                    if resp["Result"] == "TfaRequired":
                        data["TfaType"] = "normal"
                        data["TfaCode"] = cli_ui.ask_string(
                            "2FA Required: Please enter 2FA code"
                        )
                        loginresponse = session.post(
                            "https://passthepopcorn.me/ajax.php?action=login",
                            data=data,
    ...
    async def fill_upload_form(self, groupID, meta):
        ...
        if groupID is None:  # If need to make new group
            ...
            while cover is None:
                cover = cli_ui.ask_string(
                    "No Poster was found. Please input a link to a poster: \n",
                    default="",
                )
            ...
            if new_data["year"] in ["", "0", 0, None] and meta.get(
                "manual_year"
            ) not in [0, "", None]:
            ...
            while new_data["tags"] == "":
                if meta.get("mode", "discord") == "cli":
                    ...
                    new_data["tags"] = console.input(
                        "Please enter at least one tag. Comma seperated (action, animation, short):"
                    )
    ...
    async def upload(self, meta, url, data, disctype):
        ...
        if torrent.piece_size > 16777216:  # 16 MiB in bytes
            console.print(
                "[red]Piece size is OVER 16M and does not work on PTP. Generating a new .torrent"
            )
            ...
            new_torrent.metainfo["info"][
                "piece length"
            ] = 16777216  # Ensure 'piece length' is set
