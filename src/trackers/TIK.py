...
class TIK:
    ...
    async def upload(self, meta, disctype):
        ...
        cat_id = await self.get_cat_id(
            meta["category"], meta.get("foreign"), meta.get("opera"), meta.get("asian")
        )
        ...
        if self.config["TRACKERS"][self.tracker].get("personal", False) is True:
            if meta["tag"] != "" and (
                meta["tag"][1:]
                in self.config["TRACKERS"][self.tracker].get("personal_group", [])
            ):
    ...
    async def get_name(self, meta, disctype):
        ...
        meta["category_id"] = await self.get_cat_id(
            category_name, foreign, opera, asian
        )
        ...
        confirmation = cli_ui.ask_yes_no(
            "Do you want to use this name?", default=False
        )  # Default is 'No'
        ...
        if confirmation:
            ...
            console.print(
                "[red]Sorry, this seems to be an edge case, please report at (insert_link)"
            )
            sys.exit(1)
    ...
    async def edit_desc(self, meta):
        ...
        if os.path.exists(poster_jpg_path):
            ...
            console.print(
                "[green]Poster already exists as poster.jpg, skipping download.[/green]"
            )
        elif os.path.exists(poster_png_path):
            poster_path = poster_png_path
            console.print(
                "[green]Poster already exists as poster.png, skipping download.[/green]"
            )
        else:
        ...
        if meta["is_disc"] == "BDMV":
            desc_text.append(
                f"  Disc Label.........:{meta.get('bdinfo', {}).get('label', '')}\n"
            )
        desc_text.append(
            f"  IMDb...............: [url=https://www.imdb.com/title/tt{meta.get('imdb_id')}]{meta.get('imdb_rating', '')}[/url]\n"
        )
        desc_text.append(f"  Year...............: {meta.get('year', '')}\n")
        desc_text.append(f"  Country............: {country_name}\n")
        if meta["is_disc"] == "BDMV":
            desc_text.append(
                f"  Runtime............: {meta.get('bdinfo', {}).get('length', '')} hrs [color=red](double check this is actual runtime)[/color]\n"
            )
        else:
            desc_text.append(
        ...
        if meta["is_disc"] == "BDMV":
            audio_languages = ", ".join(
                [
                    ...
                    for track in meta.get("bdinfo", {}).get("audio", [])
                ]
            )
            desc_text.append(f"  Audio..............: {audio_languages}\n")
            desc_text.append(
                f"  Subtitles..........: {', '.join(meta.get('bdinfo', {}).get('subtitles', []))}\n"
            )
        else:
            ...
            for disc in meta.get("discs", []):
                ...
                if audio_section:
                    ...
                    channels = (
                        audio_section.split("Channel(s)")[1]
                        .split(":")[1]
                        .strip()
                        .split(" ")[0]
                        if "Channel(s)" in audio_section
                        else "Unknown"
                    )
                    # Convert 6 channels to 5.1, otherwise leave as is
                    channels = "5.1" if channels == "6" else channels
                    language = (
                        disc.get("ifo_mi_full", "")
                        .split("Language")[1]
                        .split(":")[1]
                        .strip()
                        .split("\n")[0]
                        if "Language" in disc.get("ifo_mi_full", "")
                        else "Unknown"
                    )
                ...
                if unique_audio:
                    desc_text.append(
                        f"  Audio..............: {', '.join(sorted(unique_audio))}\n"
                    )
                ...
                if unique_subtitles:
                    desc_text.append(
                        f"  Subtitles..........: {', '.join(sorted(unique_subtitles))}\n"
                    )
        ...
        if meta["is_disc"] == "BDMV":
            ...
            desc_text.append(
                f"  Video Format.......: {video_codec} / {video_bitrate}\n"
            )
        else:
            desc_text.append(
                f"  DVD Format.........: {meta.get('source', 'Unknown')}\n"
            )
        desc_text.append(
        ...
        if meta["is_disc"] == "BDMV":
            desc_text.append(
                f"  Source.............: {meta.get('disctype', 'Unknown')}\n"
            )
        else:
            desc_text.append(
                f"  Source.............: {meta.get('dvd_size', 'Unknown')}\n"
            )
        desc_text.append(
        ...
        if edit_choice.lower() == "e":
            ...
            console.print(
                f"Final description after editing: {description}", markup=False
            )
        else:
