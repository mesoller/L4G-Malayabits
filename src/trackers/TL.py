...
class TL:
    ...
    def __init__(self, config):
        ...
        self.config["TRACKERS"][self.tracker][
            "announce_url"
        ] = f"https://tracker.torrentleech.org/a/{self.announce_key}/announce"
        pass
    ...
    async def get_cat_id(self, common, meta):
        ...
        if meta["category"] == "MOVIE":
            if meta["original_language"] != "en":
            ...
            elif meta["is_disc"] in ("BDMV", "HDDVD") or (
                meta["type"] == "REMUX" and meta["source"] in ("BluRay", "HDDVD")
            ):
                ...
                meta["type"] == "REMUX" and "DVD" in meta["source"]
            ):
    ...
    async def upload(self, meta, disctype):
        ...
        info_filename = (
            "BD_SUMMARY_00" if meta["bdinfo"] is not None else "MEDIAINFO_CLEANPATH"
        )
        open_info = open(
            f"{meta['base_dir']}/tmp/{meta['uuid']}/{info_filename}.txt",
            "r",
            encoding="utf-8",
        )
        open_desc.write("\n\n")
        ...
        data = {"announcekey": self.announce_key, "category": cat_id}
        headers = {
