...
class NBL:
    ...
    async def search_existing(self, meta, disctype):
        ...
        json = {
            ...
            "id": 1,
            "method": "getTorrents",
            "params": [self.api_key, search_term],
        }
        try:
            ...
            for each in response["result"]["items"]:
                if meta["resolution"] in each["tags"]:
                    if meta.get("tv_pack", 0) == 1:
                        if each["cat"] == "Season" and int(
                            guessit(each["rls_name"]).get("season", "1")
                        ) == int(meta.get("season_int")):
                            dupes.append(each["rls_name"])
                    elif int(guessit(each["rls_name"]).get("episode", "0")) == int(
            ...
            if e.args[0] == "result":
                ...
                console.print(
                    "[red]NBL API Returned an unexpected response, please manually check for dupes"
                )
