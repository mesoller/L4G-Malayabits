...
class UTP:
    ...
    async def upload(self, meta, disctype):
        ...
        if meta["bdinfo"] is not None:
            ...
            bd_dump = None
        desc = open(
            f"{meta['base_dir']}/tmp/{meta['uuid']}/[UTOPIA]DESCRIPTION.txt",
            "r",
            encoding="utf-8",
        ).read()
        open_torrent = open(
            f"{meta['base_dir']}/tmp/{meta['uuid']}/[UTOPIA]{meta['clean_name']}.torrent",
            "rb",
        )
        files = {
    ...
    async def get_res_id(self, resolution):
        resolution_id = {"4320p": "1", "2160p": "2", "1080p": "3", "1080i": "4"}.get(
            resolution, "1"
        )
