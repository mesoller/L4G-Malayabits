...
class R4E:
    ...
    async def upload(self, meta, disctype):
        ...
        if (
            meta["anon"] == 0
            and bool(str2bool(str(self.config["TRACKERS"]["R4E"].get("anon", "False"))))
            is False
        ):
            anon = 0
        else:
        ...
        if meta["bdinfo"] is not None:
            ...
            bd_dump = None
        desc = open(
            f"{meta['base_dir']}/tmp/{meta['uuid']}/[R4E]DESCRIPTION.txt",
            "r",
            encoding="utf-8",
        ).read()
        open_torrent = open(
            f"{meta['base_dir']}/tmp/{meta['uuid']}/[R4E]{meta['clean_name']}.torrent",
            "rb",
        )
