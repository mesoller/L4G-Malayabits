...
class MTV:
    ...
    async def upload_with_retry(self, meta, cookiefile, common, img_host_index=1):
        ...
        if all(
            ...
            console.print(
                "[green]Images are already hosted on an approved image host. Skipping re-upload."
            )
            ...
            while img_host_index <= len(approved_image_hosts):
                ...
                image_list, retry_mode = await self.handle_image_upload(
                    meta, img_host_index, approved_image_hosts
                )

                # If retry_mode is True, switch to the next host
                if retry_mode:
                    console.print(
                        f"[yellow]Switching to the next image host. Current index: {img_host_index}"
                    )
                    img_host_index += 1
                    continue
        ...
        if torrent.piece_size > 8388608:  # 8 MiB in bytes
            ...
            meta["max_piece_size"] = (
                "8"  # 8 MiB, to ensure the new torrent adheres to this limit
            )
            ...
            if meta["is_disc"]:
                include = (
                    []
                )  # Adjust as needed for disc-specific inclusions, make sure it's a list
                exclude = (
                    []
                )  # Adjust as needed for disc-specific exclusions, make sure it's a list
            else:
                include = ["*.mkv", "*.mp4", "*.ts"]
            ...
            new_torrent.write(
                f"{meta['base_dir']}/tmp/{meta['uuid']}/MTV.torrent", overwrite=True
            )
                ...
                try:
                    if "torrents.php" in response.url:
                        ...
                        if "authkey.php" in response.url:
                            console.print(
                                "[red]No DL link in response, It may have uploaded, check manually."
                            )
                        else:
                            console.print(
                                "[red]Upload Failed. It doesn't look like you are logged in."
                            )
                except Exception:
    ...
    async def handle_image_upload(
        self, meta, img_host_index=1, approved_image_hosts=None
    ):
        ...
        if not current_img_host or current_img_host not in approved_image_hosts:
            console.print(
                "[red]Your preferred image host is not supported at MTV, re-uploading to an allowed image host."
            )
            retry_mode = True  # Ensure retry_mode is set to True when switching hosts
            meta["imghost"] = approved_image_hosts[
                0
            ]  # Switch to the first approved host
        else:
            meta["imghost"] = current_img_host
            retry_mode = (
                False  # Start with retry_mode False unless we know we need to switch
            )
        ...
        prep.screenshots(
            Path(meta["path"]), meta["name"], meta["uuid"], meta["base_dir"], meta
        )
        return_dict = {}
        ...
        if not all(
            any(x in image["raw_url"] for x in approved_image_hosts)
            for image in meta["image_list"]
        ):
            console.print(
                "[red]Unsupported image host detected, please use one of the approved image hosts"
            )
    ...
    async def get_tags(self, meta):
        ...
        # Genres
        tags.extend(
            [x.strip(", ").lower().replace(" ", ".") for x in meta["genres"].split(",")]
        )
        # Resolution
    ...
    async def search_existing(self, meta, disctype):
        ...
        try:
            ...
            console.print(
                "[red]Unable to search for existing torrents on site. Most likely the site is down."
            )
