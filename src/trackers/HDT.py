...
class HDT:
    ...
    async def upload(self, meta, disctype):
        ...
        if meta.get("unattended", False) is False:
            ...
            if hdt_confirm is not True:
                hdt_name_manually = cli_ui.ask_string(
        ...
        with open(torrent_path, "rb") as torrentFile:
            ...
            data = {"filename": hdt_name, "category": cat_id, "info": hdt_desc.strip()}
            ...
            if meta["debug"]:
                ...
                with requests.Session() as session:
                    cookiefile = os.path.abspath(
                        f"{meta['base_dir']}/data/cookies/HDT.txt"
                    )
                    ...
                    search = re.search(
                        r"download\.php\?id\=([a-z0-9]+)", up.text
                    ).group(1)
                    if search:
                        ...
                        await common.add_tracker_torrent(
                            ...
                            self.tracker,
                            self.source_flag,
                            self.config["TRACKERS"]["HDT"].get("my_announce_url"),
                            "https://hd-torrents.org/details.php?id=" + search,
                        )
                    else:
                        console.print(data)
                        console.print("\n\n")
    ...
    async def validate_credentials(self, meta):
        ...
        if vcookie is not True:
            console.print(
                "[red]Failed to validate cookies. Please confirm that the site is up or export a fresh cookie file from the site"
            )
            return False
        return True
    ...
    async def edit_desc(self, meta):
        ...
        with open(
            ...
            if meta["is_disc"] != "BDMV":
                ...
                mi_template = os.path.abspath(
                    f"{meta['base_dir']}/data/templates/MEDIAINFO.txt"
                )
