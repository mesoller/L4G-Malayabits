...
class HDB:
    ...
    async def get_tags(self, meta):
        ...
        if meta.get("silent", False) is True:
            console.print(
                "[yellow]zxx audio track found, suggesting you tag as silent"
            )  # 57
    ...
    async def edit_name(self, meta):
        ...
        if (
            meta.get("source", "").upper() == "WEB"
            and meta.get("service", "").strip() != ""
        ):
        ...
        if meta.get("type") in ("WEBDL", "WEBRIP", "ENCODE"):
            hdb_name = hdb_name.replace(
                meta["audio"], meta["audio"].replace(" ", "", 1).replace("Atmos", "")
            )
        else:
            hdb_name = hdb_name.replace(
                meta["audio"], meta["audio"].replace("Atmos", "")
            )
        ...
        if meta.get("imdb_info"):
            ...
            if (
                str(meta["year"])
                != str(meta.get("imdb_info", {}).get("year", meta["year"]))
                and str(meta["year"]).strip() != ""
            ):
                hdb_name = hdb_name.replace(
                    str(meta["year"]), str(meta["imdb_info"]["year"])
                )
    ...
    async def upload(self, meta, disctype):
        ...
        for each in (cat_id, codec_id, medium_id):
            if each == "EXIT":
                console.print(
                    "[bold red]Something didn't map correctly, or this content is not allowed on HDB"
                )
                return
        if "Dual-Audio" in meta["audio"] and meta["is_disc"] not in (
            "BDMV",
            "HDDVD",
            "DVD",
        ):
        ...
        if torrent.piece_size > 16777216:  # 16 MiB in bytes
            console.print(
                "[red]Piece size is OVER 16M and does not work on HDB. Generating a new .torrent"
            )
        ...
        with open(torrent_path, "rb") as torrentFile:
            ...
            if meta["debug"]:
                ...
                with requests.Session() as session:
                    ...
                    match = re.match(
                        r".*?hdbits\.org/details\.php\?id=(\d+)&uploaded=(\d+)", up.url
                    )
                    if match:
                        ...
                        raise UploadException(
                            f"Upload to HDB Failed: result URL {up.url} ({up.status_code}) was not expected",
                            "red",
                        )  # noqa F405
        return
    ...
    async def search_existing(self, meta, disctype):
        ...
        try:
            ...
            console.print(
                "[bold red]Unable to search for existing torrents on site. Either the site is down or your passkey is incorrect"
            )
    ...
    async def validate_credentials(self, meta):
        ...
        if vapi is not True:
            console.print(
                "[red]Failed to validate API. Please confirm that the site is up and your passkey is valid."
            )
            return False
    ...
    async def validate_api(self):
        ...
        data = {"username": self.username, "passkey": self.passkey}
        try:
    ...
    async def edit_desc(self, meta):
        ...
        with open(
            ...
            if (
                ...
                descfile.write(
                    f"[center][quote]This release is sourced from {meta['service_longname']}[/quote][/center]"
                )
            bbcode = BBCODE()
            if meta.get("discs", []) != []:
                discs = meta["discs"]
                if discs[0]["type"] == "DVD":
                    descfile.write(
                        f"[quote=VOB MediaInfo]{discs[0]['vob_mi']}[/quote]\n"
                    )
                    descfile.write("\n")
                ...
                if len(discs) >= 2:
                    for each in discs[1:]:
                        if each["type"] == "BDMV":
                            descfile.write(
                                f"[quote={each.get('name', 'BDINFO')}]{each['summary']}[/quote]\n"
                            )
                            descfile.write("\n")
                            pass
                        if each["type"] == "DVD":
    ...
    async def hdbimg_upload(self, meta):
        ...
        hdbimg_screen_count = (
            3 if meta["category"] == "TV" and meta.get("tv_pack", 0) == 0 else 6
        )
    ...
    async def get_info_from_torrent_id(self, hdb_id):
        ...
        if response.ok:
            ...
            console.print(
                "Failed to get info from HDB ID. Either the site is down or your credentials are invalid"
            )
    ...
    async def search_filename(self, search_term, search_file_folder, meta):
        ...
        if search_file_folder == "folder" and meta.get("is_disc"):
            bd_summary_path = os.path.join(
                meta["base_dir"], "tmp", meta["uuid"], "BD_SUMMARY_00.txt"
            )
            bd_summary = None
            ...
            try:
                ...
                if bd_summary:
                    ...
                    console.print(
                        f"[green]Searching HDB for disc title: [bold yellow]{bd_summary}[/bold yellow]"
                    )
                    # console.print(f"[yellow]Using this data: {data}")
                else:
                    console.print(
                        f"[red]Error: 'Disc Title' not found in {bd_summary_path}[/red]"
                    )
            ...
            console.print(
                f"[green]Searching HDB for file: [bold yellow]{os.path.basename(search_term)}[/bold yellow]"
            )
            # console.print(f"[yellow]Using this data: {data}")
        ...
        if response.ok:
            try:
                ...
                if "data" not in response_json:
                    console.print(
                        f"[red]Error: 'data' key not found in HDB API response. Full response: {response_json}[/red]"
                    )
                    return hdb_imdb, hdb_tvdb, hdb_name, hdb_torrenthash, hdb_id
                ...
                if response_json["data"] != []:
                    for each in response_json["data"]:
                        ...
                        hdb_id = each["id"]
                        console.print(
                            f"[bold green]Matched release with HDB ID: [yellow]https://hdbits.org/details.php?id={hdb_id}[/yellow][/bold green]"
                        )
                        return hdb_imdb, hdb_tvdb, hdb_name, hdb_torrenthash, hdb_id
                else:
                    console.print(
                        "[yellow]No data found in the HDB API response[/yellow]"
                    )
            except Exception as e:
                console.print_exception()
                console.print(
                    f"[red]Failed to parse HDB API response. Error: {str(e)}[/red]"
                )
        else:
            console.print(
                f"[red]Failed to get info from HDB. Status code: {response.status_code}, Reason: {response.reason}[/red]"
            )
