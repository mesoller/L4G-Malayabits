...
class Clients:
    ...
    async def find_existing_torrent(self, meta):
        ...
        if torrent_storage_dir is None and torrent_client != "watch":
            console.print(
                f"[bold red]Missing torrent_storage_dir for {default_torrent_client}"
            )
            return None
        elif not os.path.exists(str(torrent_storage_dir)) and torrent_client != "watch":
            console.print(
                f"[bold red]Invalid torrent_storage_dir path: [bold yellow]{torrent_storage_dir}"
            )
        torrenthash = None
        if torrent_storage_dir is not None and os.path.exists(torrent_storage_dir):
            if meta.get("torrenthash", None) is not None:
                valid, torrent_path = await self.is_valid_torrent(
                    meta,
                    f"{torrent_storage_dir}/{meta['torrenthash']}.torrent",
                    meta["torrenthash"],
                    torrent_client,
                    print_err=True,
                )
                ...
                valid, torrent_path = await self.is_valid_torrent(
                    meta,
                    f"{torrent_storage_dir}/{meta['ext_torrenthash']}.torrent",
            ...
            if (
                torrent_client == "qbit"
                and torrenthash is None
                and client.get("enable_search") is True
            ):
    ...
    async def is_valid_torrent(
        self, meta, torrent_path, torrenthash, torrent_client, print_err=False
    ):
        valid = False
        wrong_file = False
        err_print = ""
        ...
        if os.path.exists(torrent_path):
            ...
            if meta.get("is_disc", None) is not None or (
            ...
            if len(torrent.files) == len(meta["filelist"]) == 1:
                if os.path.basename(torrent.files[0]) == os.path.basename(
                    meta["filelist"][0]
                ):
                ...
                if meta["debug"]:
                    console.log(
                        f"Single file match status: valid={valid}, wrong_file={wrong_file}"
                    )
            ...
            # Check if number of files matches number of videos
                ...
                if (
                    local_path.lower() in meta["path"].lower()
                    and local_path.lower() != remote_path.lower()
                ):
                    actual_filepath = actual_filepath.replace(
        ...
        if valid:
            if os.path.exists(torrent_path):
                try:
                    ...
                    if meta["debug"]:
                        console.log(
                            f"Checking piece size and count: pieces={reuse_torrent.pieces}, piece_size={reuse_torrent.piece_size}"
                        )

                    # Piece size and count validations
                    if (
                        reuse_torrent.pieces >= 7000
                        and reuse_torrent.piece_size < 8388608
                    ) or (
                        reuse_torrent.pieces >= 4000
                        and reuse_torrent.piece_size < 4194304
                    ):
    ...
    async def search_qbit_for_torrent(self, meta, client):
        ...
        if (
            torrent_storage_dir is None
            and client.get("torrent_client", None) != "watch"
        ):
            console.print(
                f"[bold red]Missing torrent_storage_dir for {self.config['DEFAULT']['default_torrent_client']}"
            )
            return None
        ...
        for torrent in torrents:
            try:
                torrent_path = torrent.get(
                    "content_path", f"{torrent.save_path}{torrent.name}"
                )
            ...
            if meta["is_disc"] in ("", None) and len(meta["filelist"]) == 1:
                if torrent_path == meta["filelist"][0] and len(torrent.files) == len(
                    meta["filelist"]
                ):
                    valid, torrent_path = await self.is_valid_torrent(
                        meta,
                        f"{torrent_storage_dir}/{torrent.hash}.torrent",
                        torrent.hash,
                        "qbit",
                        print_err=False,
                    )
                    if valid:
                        console.print(
                            f"[green]Found a matching .torrent with hash: [bold yellow]{torrent.hash}"
                        )
                        return torrent.hash
    ...
    def rtorrent(
        self, path, torrent_path, torrent, meta, local_path, remote_path, client
    ):
        rtorrent = xmlrpc.client.Server(
            client["rtorrent_url"], context=ssl._create_stdlib_context()
        )
        ...
        modified_fr = False
        if (
            local_path.lower() in path.lower()
            and local_path.lower() != remote_path.lower()
        ):
    ...
    async def qbittorrent(
        self, path, torrent, local_path, remote_path, client, is_disc, filelist, meta
    ):
        ...
        if isinstance(am_config, list):
            ...
            if (
                os.path.normpath(am_config).lower() in os.path.normpath(path).lower()
                and am_config.strip() != ""
            ):
                auto_management = True
        qbt_category = (
            client.get("qbit_cat") if not meta.get("qbit_cat") else meta.get("qbit_cat")
        )
        ...
        if client.get("qbit_tag", None) is not None:
            qbt_client.torrents_add_tags(
                tags=client.get("qbit_tag"), torrent_hashes=torrent.infohash
            )
        if meta.get("qbit_tag") is not None:
            qbt_client.torrents_add_tags(
                tags=meta.get("qbit_tag"), torrent_hashes=torrent.infohash
            )
        console.print(f"Added to: {path}")

    def deluge(
        self, path, torrent_path, torrent, local_path, remote_path, client, meta
    ):
    ...
    async def remote_path_map(self, meta):
        ...
        local_path = list_local_path = self.config["TORRENT_CLIENTS"][
            torrent_client
        ].get("local_path", "/LocalPath")
        remote_path = list_remote_path = self.config["TORRENT_CLIENTS"][
            torrent_client
        ].get("remote_path", "/RemotePath")
