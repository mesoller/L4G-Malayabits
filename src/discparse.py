        ...
        for i in range(len(discs)):
            ...
            if bdinfo_text is None or meta_discs == []:
                if os.path.exists(f"{save_dir}/BD_FULL_{str(i).zfill(2)}.txt"):
                    bdinfo_text = os.path.abspath(
                        f"{save_dir}/BD_FULL_{str(i).zfill(2)}.txt"
                    )
                else:
                    bdinfo_text = ""
                    if sys.platform.startswith("linux") or sys.platform.startswith(
                        ...
                        try:
                            ...
                            proc = await asyncio.create_subprocess_exec(
                                "mono",
                                f"{base_dir}/bin/BDInfo/BDInfo.exe",
                                "-w",
                                path,
                                save_dir,
                            )
                            ...
                            console.print(
                                "[bold red]mono not found, please install mono"
                            )
                        ...
                        console.print(f"[bold green]Scanning {path}")
                        proc = await asyncio.create_subprocess_exec(
                            f"{base_dir}/bin/BDInfo/BDInfo.exe", "-w", path, save_dir
                        )
                        await proc.wait()
                        await asyncio.sleep(1)
                    else:
                        console.print(
                            "[red]Not sure how to run bdinfo on your platform, get support please thanks."
                        )
                while True:
                    try:
                        if bdinfo_text == "":
                        ...
                        try:
                            shutil.copyfile(
                                bdinfo_text, f"{save_dir}/BD_FULL_{str(i).zfill(2)}.txt"
                            )
                ...
                with open(f"{save_dir}/BD_SUMMARY_{str(i).zfill(2)}.txt", "w") as f:
                    ...
                    f.close()
                with open(
                    f"{save_dir}/BD_SUMMARY_EXT.txt", "w"
                ) as f:  # write extended BDInfo file
    ...
    async def get_dvdinfo(self, discs):
        for each in discs:
            ...
            for vob_set in filesdict.values():
                ...
                vob_set_mi = MediaInfo.parse(
                    f"VTS_{vob_set[0][:2]}_0.IFO", output="JSON"
                )
                ...
                if (float(vob_set_duration) * 1.00) > (
                    float(main_set_duration) * 1.10
                ) or len(main_set) < 1:
                    main_set = vob_set
            ...
            each["vob_mi"] = MediaInfo.parse(
                os.path.basename(vob),
                output="STRING",
                full=False,
                mediainfo_options={"inform_version": "1"},
            ).replace("\r\n", "\n")
            each["ifo_mi"] = MediaInfo.parse(
                os.path.basename(ifo),
                output="STRING",
                full=False,
                mediainfo_options={"inform_version": "1"},
            ).replace("\r\n", "\n")
            each["vob_mi_full"] = MediaInfo.parse(
                vob,
                output="STRING",
                full=False,
                mediainfo_options={"inform_version": "1"},
            ).replace("\r\n", "\n")
            each["ifo_mi_full"] = MediaInfo.parse(
                ifo,
                output="STRING",
                full=False,
                mediainfo_options={"inform_version": "1"},
            ...
            size = sum(
                os.path.getsize(f) for f in os.listdir(".") if os.path.isfile(f)
            ) / float(1 << 30)
            if size <= 7.95:
                dvd_size = "DVD9"
                if size <= 4.37:
