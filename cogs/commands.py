...
class Commands(commands.Cog):
    ...
    async def upload(self, ctx, path, *args, message_id=0, search_args=tuple()):
        ...
        if os.path.exists(path):
            ...
            prep = Prep(
                screens=meta["screens"], img_host=meta["imghost"], config=config
            )
    ...
    async def search(self, ctx, *, args=None):
        ...
        if not files_total:
            ...
            embed = discord.Embed(
                title=f"File search results for: `{search_terms}`",
                color=0x00FF40,
                description=f"```• {files}```",
            )
            embed.add_field(
                name="What Now?",
                value=f"Please be more specific or use `{config['DISCORD']['command_prefix']}search dir` to find a directory",
            )
            ...
            embed.set_footer(
                text=f"{config['DISCORD']['discord_emojis']['UPLOAD']} to Upload"
            )
            message = await ctx.send(embed=embed)
    ...
    async def send_embed_and_upload(self, ctx, meta):
        ...
        if meta.get("uploaded_screens", False) is False:
            if meta.get("embed_msg_id", "0") != "0":
                ...
                await message.edit(
                    embed=discord.Embed(title="Uploading Screenshots", color=0xFFFF00)
                )
            else:
            ...
            u = multiprocessing.Process(
                target=prep.upload_screens,
                args=(meta, meta["screens"], 1, 0, meta["screens"], [], return_dict),
            )
            u.start()
        ...
        if len(glob(f"{meta['base_dir']}/tmp/{meta['uuid']}/BASE.torrent")) == 0:
            if meta.get("embed_msg_id", "0") != "0":
                ...
                await message.edit(
                    embed=discord.Embed(title="Creating .torrent", color=0xFFFF00)
                )
            else:
                message = await ctx.send(
                    embed=discord.Embed(title="Creating .torrent", color=0xFFFF00)
            ...
            if meta["nohash"] is False:
                ...
                p = multiprocessing.Process(
                    target=prep.create_torrent, args=(meta, Path(meta["path"]))
                )
                p.start()
        ...
        embed.add_field(
            name="Links",
            value=f"[TMDB](https://www.themoviedb.org/{meta['category'].lower()}/{meta['tmdb']}){imdb}{tvdb}",
        )
        embed.add_field(
            name=f"{res} / {meta['type']}{tag}",
            value=f"```{meta['name']}```",
            inline=False,
        )
        if missing != []:
            embed.add_field(
                name="POTENTIALLY MISSING INFORMATION:",
                value="\n".join(missing),
                inline=False,
            )
        ...
        embed.set_author(
            name="L4G's Upload Assistant",
            url="https://github.com/Audionut/Upload-Assistant",
            icon_url="https://images2.imgbox.com/6e/da/dXfdgNYs_o.png",
        )
        ...
        for each in trackers:
            ...
            if "AITHER" in each.replace(" ", ""):
                await message.add_reaction(
                    config["DISCORD"]["discord_emojis"]["AITHER"]
                )
                await asyncio.sleep(0.3)
        ...
        def check(reaction, user):
            if reaction.message.id == meta["embed_msg_id"]:
                if str(user.id) == config["DISCORD"]["admin_id"]:
                    ...
                    if (
                        str(reaction.emoji)
                        == config["DISCORD"]["discord_emojis"]["CANCEL"]
                    ):
        ...
        try:
            ...
            try:
                ...
                timeout_embed = discord.Embed(
                    title=f"{meta['title']} has timed out", color=0xFF0000
                )
            ...
            cancel_embed = discord.Embed(
                title=f"{meta['title']} has been cancelled", color=0xFF0000
            )
            await msg.clear_reactions()
            ...
            while not tracker_list:
                ...
                for each in msg.reactions:
                    if each.count >= 2:
                        tracker = list(config["DISCORD"]["discord_emojis"].keys())[
                            list(config["DISCORD"]["discord_emojis"].values()).index(
                                str(each)
                            )
                        ]
            ...
            upload_embed = discord.Embed(
                title=f"Uploading `{meta['name']}` to:",
                description=upload_embed_description,
                color=0x00FF40,
            )
            ...
            if "MANUAL" in tracker_list:
                ...
                upload_embed_description = upload_embed_description.replace(
                    "MANUAL", "~~MANUAL~~"
                )
                if archive_url is False:
                    upload_embed = discord.Embed(
                        title=f"Uploaded `{meta['name']}` to:",
                        description=upload_embed_description,
                        color=0xFF0000,
                    )
                    upload_embed.add_field(
                        name="Unable to upload prep files",
                        value=f"The files can be found at `tmp/{meta['title']}.tar`",
                    )
                    await msg.edit(embed=upload_embed)
                else:
                    upload_embed = discord.Embed(
                    ...
                    upload_embed.add_field(
                        name="Files can be found at:",
                        value=f"{archive_url} or `tmp/{meta['uuid']}`",
                    )
                    await msg.edit(embed=upload_embed)
    ...
    async def dupe_embed(self, dupes, meta, emojis, channel):
        if not dupes:
            ...
            embed = discord.Embed(
                title="Check if these are actually dupes!",
                description=dupe_text,
                color=0xFF0000,
            )
            embed.set_footer(
                text=f"{emojis['CANCEL']} to abort upload | {emojis['UPLOAD']} to upload anyways"
            )
    ...
    async def get_missing(self, meta):
        ...
        if meta.get("imdb_id", "0") == "0":
            missing.append("--imdb")
        if (
            isinstance(meta["potential_missing"], list)
            and len(meta["potential_missing"]) > 0
        ):
