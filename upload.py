...
def get_confirmation(meta):
    ...
    if meta.get('unattended', False) == False:
        ...
        if meta['isdir'] and meta['keep_folder']:
            cli_ui.info_section(cli_ui.yellow, f"Uploading with -kf/--keep-folder")
            kf_confirm = cli_ui.ask_yes_no("You specified -kf/--keep-folder. Uploading in folders might not be allowed. Are you sure you want to proceed?", default=False)
            if not kf_confirm:
