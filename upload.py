    ...
    if not os.path.exists(os.path.abspath(f"{base_dir}/data/config.py")):
        try:
            if os.path.exists(os.path.abspath(f"{base_dir}/data/config.json")):
                with open(
                    f"{base_dir}/data/config.json", "r", encoding="utf-8-sig"
                ) as f:
                ...
                with open(f"{base_dir}/data/config.py", "w") as f:
                    ...
                    f.close()
                cli_ui.info(
                    cli_ui.green, "Successfully updated config from .json to .py"
                )
                cli_ui.info(
                    cli_ui.green,
                    "It is now safe for you to delete",
                    cli_ui.yellow,
                    "data/config.json",
                    "if you wish",
                )
            ...
            cli_ui.info(
                ...
                "to",
                cli_ui.green,
                os.path.abspath(f"{base_dir}/data/config.py"),
            )
            cli_ui.info(
                cli_ui.red,
                "2: Add",
...
async def do_the_thing(base_dir):
    ...
    for each in sys.argv[1:]:
        if os.path.exists(each):
            ...
            break
    meta, help, before_args = parser.parse(
    ...
    if os.path.exists(path):
        ...
        if os.path.exists(os.path.dirname(path)) and len(paths) <= 1:
            ...
            for i, each in enumerate(split_path):
                try:
                    if os.path.exists(p1) and not os.path.exists(
                        f"{p1} {split_path[i + 1]}"
                    ):
                        queue.append(p1)
                        p1 = split_path[i + 1]
                    ...
                    if os.path.exists(p1):
                        ...
                        console.print(
                            f"[red]Path: [bold red]{p1}[/bold red] does not exist"
                        )
            if len(queue) >= 1:
                md_text = "\n - ".join(queue)
                console.print("\n[bold green]Queuing these files:[/bold green]", end="")
            ...
            # Add Search Here
            console.print(
                "[red]There was an issue with your input. If you think this was not an issue, please make a report that includes the full command used."
            )
            exit()
    ...
    for path in queue:
        ...
        ) = await prep.get_name(meta)

        if (
            meta.get("image_list", False) in (False, [])
            and meta.get("skip_imghost_upload", False) is False
        ):
            return_dict = {}
            meta["image_list"], dummy_var = prep.upload_screens(
                meta, meta["screens"], 1, 0, meta["screens"], [], return_dict
            )
            if meta["debug"]:
                console.print(meta["image_list"])
            # meta['uploaded_screens'] = True
        elif (
            meta.get("skip_imghost_upload", False) is True
            and meta.get("image_list", False) is False
        ):
            meta["image_list"] = []

        if not os.path.exists(
            os.path.abspath(f"{meta['base_dir']}/tmp/{meta['uuid']}/BASE.torrent")
        ):
            reuse_torrent = None
            if meta.get("rehash", False) is False:
                reuse_torrent = await client.find_existing_torrent(meta)
                if reuse_torrent is not None:
                    prep.create_base_from_existing_torrent(
            ...
            and meta.get("rehash", False) is True
            and meta["nohash"] is False
        ):
            prep.create_torrent(meta, Path(meta["path"]), "BASE")
        if int(meta.get("randomized", 0)) >= 1:
            prep.create_random_torrents(
                meta["base_dir"], meta["uuid"], meta["randomized"], meta["path"]
            )
        ...
        for tracker in trackers:
            ...
            if tracker in api_trackers:
                ...
                if upload_to_tracker:
                    ...
                    modq, draft = await check_mod_q_and_draft(
                        ...
                        if manual_tracker != "MANUAL":
                            ...
                            if manual_tracker in api_trackers:
                                await common.unit3d_edit_desc(
                                    meta, tracker_class.tracker, tracker_class.signature
                                )
                            else:
                ...
                if meta["unattended"]:
                    ...
                    try:
                        upload_to_ptp = cli_ui.ask_yes_no(
                            f"Upload to THR? {debug}", default=meta["unattended"]
                        )
                    except (KeyboardInterrupt, EOFError):
                        sys.exit(1)  # Exit immediately
                if upload_to_thr:
                    ...
                    if meta.get("youtube", None) is None:
                        youtube = cli_ui.ask_string(
                            "Unable to find youtube trailer, please link one e.g.(https://www.youtube.com/watch?v=dQw4w9WgXcQ)"
                        )
                    ...
                    try:
                        with requests.Session() as session:
                            ...
                            dupes = thr.search_existing(
                                session, disctype, meta.get("imdb_id")
                            )
                            dupes = await common.filter_dupes(dupes, meta)
                            meta = dupe_check(dupes, meta)
                            if meta["upload"] is True:
            ...
            if tracker == "PTP":
                if meta["unattended"]:
                    ...
                    try:
                        upload_to_ptp = cli_ui.ask_yes_no(
                            f"Upload to {tracker}? {debug}", default=meta["unattended"]
                        )
                    except (KeyboardInterrupt, EOFError):
                        sys.exit(1)  # Exit immediately
                ...
                if upload_to_ptp:  # Ensure the variable is defined before this check
                    ...
                    try:
                        ...
                        if groupID is None:
                            ...
                            if meta.get(
                                "youtube", None
                            ) is None or "youtube" not in str(meta.get("youtube", "")):
                                youtube = cli_ui.ask_string(
                                    "Unable to find youtube trailer, please link one e.g.(https://www.youtube.com/watch?v=dQw4w9WgXcQ)",
                                    default="",
                                )
                        ...
                        if meta.get("imdb_info", {}) == {}:
                            meta["imdb_info"] = await prep.get_imdb_info(
                                meta["imdb_id"], meta
                            )
                        if meta["upload"] is True:
...
def get_confirmation(meta):
    ...
    if meta.get("unattended", False) is False:
        get_missing(meta)
        ring_the_bell = (
            "\a" if config["DEFAULT"].get("sfx_on_prompt", True) is True else ""
        )  # \a rings the bell
        ...
        if meta.get("is disc", False):
            meta["keep_folder"] = (
                False  # Ensure 'keep_folder' is False if 'is disc' is True
            )
            ...
            if "keep_folder" in meta:
                if meta["keep_folder"]:
                    ...
                    kf_confirm = cli_ui.ask_yes_no(
                        "You specified --keep-folder. Uploading in folders might not be allowed. Are you sure you want to proceed?",
                        default=False,
                    )
                    if not kf_confirm:
    ...
    if not dupes:
        ...
        if meta["unattended"]:
            if meta.get("dupe", False) is False:
                console.print(
                    "[red]Found potential dupes. Aborting. If this is not a dupe, or you would like to upload anyways, pass --skip-dupe-check"
                )
                upload = False
            else:
                console.print(
                    "[yellow]Found potential dupes. --skip-dupe-check was passed. Uploading anyways"
                )
                upload = True
        console.print()
        if not meta["unattended"]:
        ...
        for tag in banned_group_list:
            if isinstance(tag, list):
                if meta["tag"][1:].lower() == tag[0].lower():
                    console.print(
                        f"[bold yellow]{meta['tag'][1:]}[/bold yellow][bold red] was found on [bold yellow]{tracker}'s[/bold yellow] list of banned groups."
                    )
...
if __name__ == "__main__":
    ...
    if int(pyver[0]) != 3 or int(pyver[1]) < 12:
        console.print(
            "[bold red]Python version is too low. Please use Python 3.12 or higher."
        )
        sys.exit(1)
